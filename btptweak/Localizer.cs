...
namespace BtpTweak {
    ...
    public sealed class Localizer : IEventHandlers {
        ...
        public void 基础汉化() {
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_BARRIERONOVERHEAL_DESC", $"增加{50.ToBaseAndStk().ToHealing("_点")}护甲。过量<style=cIsHealing>治疗</style>会使你获得一道相当于{0.5f.ToBaseAndStkPct().ToHealing()}已<style=cIsHealing>治疗</style>生命值的<style=cIsHealing>临时屏障</style>。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_THORNS_DESC", $"受伤时弹射出1发{"剃刀".ToDmg()}还击攻击者，若没有，则弹射到{ThornsTweak.Radius.ToBaseAndStk().ToDmg("_米")}内最近的敌人，合计造成受到的<style=cIsDamage>{ThornsTweak.BaseDamageCoefficient.ToDmgPct()}的伤害</style>。{0.1f.ToStk("（发射间隔：_秒）")}", "zh-CN");
        ...
        public void 权杖技能汉化() {
            ...
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_MERC_EVISPROJDESC", Language.GetString("MERC_SPECIAL_ALT1_DESCRIPTION") + $"额外发射{1.ToBaseAndStk().ToDmg("_片")}刀刃之风。".ToScepterDesc(), "zh-CN");
