...
namespace BtpTweak {
    ...
    internal static class Localizer {
        ...
        private static void 基础汉化() {
            ...
            //AddOverlay("ITEM_FIREWORK_DESC", $"{"激活装备".ToUtil()}时向{"烟花发射池".ToUtil()}里添加{FireworkTweak.FireCount.ToBaseAndStk().ToDmg()}枚烟花{0.125f.ToStk("（发射间隔：_秒）")}。每枚合计造成{FireworkTweak.BaseDamageCoefficient.ToDmgPct()}的伤害。", "zh-CN");
            AddOverlay("EQUIPMENT_SAWMERANG_DESC", $"投掷<style=cIsDamage>三个穿透性的回旋锯</style>，每个造成{SawTweak.DamageCoefficient.ToDmgPct("_的基础伤害")}，同时锯伤敌人，造成额外的<style=cIsDamage>每秒100％的基础伤害</style>，并使其{流血}，造成{BleedTweak.BleedDamageCoefficient.ToDmgPct("每秒_的基础伤害")}，持续{BleedTweak.BleedDuration.ToUtil("_秒")}；若{暴击}，则{额外}造成{出血}，造成{BleedTweak.SurperBleedDamageCoefficient.ToDmgPct("每秒_的基础伤害")}，持续{BleedTweak.SurperBleedDuration.ToUtil("_秒")}", "zh-CN");
            AddOverlay("KEYWORD_SUPERBLEED", $"<style=cKeywordName>出血</style><style=cSub>造成每秒{BleedTweak.SurperBleedDamageCoefficient.ToDmgPct()}的基础伤害。<i>出血可以叠加。</i></style>", "zh-CN");
            AddOverlay("BUFF_SUPERBLEED_DESC", $"每秒受到{BleedTweak.SurperBleedDamageCoefficient.ToDmgPct()}的基础伤害。", "zh-CN");
            ...
            AddOverlay("ITEM_LASERTURBINE_DESC", $"获得一个{"自动".ToUtil()}充能的共鸣圆盘，基础充能速率为{LaserTurbineTweak.ChargeCoefficient.ToBaseAndStkPct().ToUtil("_每秒")}，击杀敌人将给予一层持续{LaserTurbineTweak.ChargeDuration.ToUtil("_秒的充能增益")}，每层增益使充能速率增加{LaserTurbineTweak.ChargeCoefficientPerKill.ToPct().ToUtil("_每秒")}。圆盘充能完毕后{"自动".ToUtil()}发射，在{"可视范围内".ToUtil()}所有个体之间弹射，每一次弹射可造成{FireMainBeamState.mainBeamDamageCoefficient.ToBaseAndStkPct().ToDmg()}的基础伤害。弹射时圆盘将{"穿透".ToUtil()}沿途所有个体并产生{爆炸}，{爆炸}造成{FireMainBeamState.secondBombDamageCoefficient.ToBaseAndStkPct().ToDmg()}的基础伤害。", "zh-CN");
            AddOverlay("ITEM_ICICLE_DESC", $"召唤{"寒冰风暴".ToIce()}攻击附近的{IcicleTweak.BaseRadius.ToUtil("_米") + IcicleTweak.RadiusPerIcicle.ToStk("（每层风暴+_米）")}敌人，造成<style=cIsDamage>每秒{IcicleTweak.DamageCoefficient.ToDmgPct() + IcicleTweak.DamageCoefficientPerIcicle.ToStkPct("（每层风暴+_）")}的伤害</style>，并使敌人<style=cIsUtility>减速</style><style=cIsUtility>80%</style>。<style=cIsDamage>每达成一次击杀</style>，增加一层持续{IcicleTweak.IcicleDuration.ToBaseAndStk().ToUtil("_秒")}的{"风暴".ToIce()}，最低为{IcicleTweak.BaseIcicleMin.ToBaseWithStk(IcicleTweak.StacIcicleMin).ToUtil("_层")}，最高为{IcicleTweak.BaseIcicleMax.ToBaseWithStk(IcicleTweak.StackicicleMax).ToUtil("_层")}。", "zh-CN");
            AddOverlay("ITEM_DRONEWEAPONS_DESC", $"获得{"继承你的所有物品".ToUtil()}的<style=cIsDamage>德鲁曼上校</style>，持续{60.ToUtil("_秒")}，每次死亡后需要充能{60.ToUtil("_秒") + "（每层减少充能时间）".ToStk()}才能复活。\n无人机获得<style=cIsDamage>+50%</style><style=cStack>（每层+50%）</style>攻击速度和冷却时间缩减。\n无人机有<style=cIsDamage>10%</style>的几率在命中时发射一枚<style=cIsDamage>导弹</style>，合计造成<style=cIsDamage>300%</style>的伤害。\n无人机获得<style=cIsDamage>自动链式机枪</style>，造成<style=cIsDamage>6x100%</style>的伤害，可弹射命中<style=cIsDamage>2</style>名敌人。", "zh-CN");
            ...
            AddOverlay("CAPTAIN_UTILITY_ALT1_DESCRIPTION", $"<style=cIsDamage>眩晕</style>。{"消耗4层充能".ToUtil()}向<style=cIsDamage>UES顺风号</style>请求发动一次<style=cIsDamage>动能打击</style>。在<style=cIsUtility>20秒</style>后，对所有角色造成{CaptainTweak.CallAirstrikeAltDamageCoefficient.ToDmgPct()}的伤害。", "zh-CN");
            ...
            AddOverlay("COMMANDO_SECONDARY_DESCRIPTION", $"发射一枚<style=cIsDamage>穿甲弹</style>，造成{CommandoTweak.FMJDamageCoefficient.ToDmgPct()}的伤害。每次穿透敌人，造成的伤害提高<style=cIsDamage>40%</style>。", "zh-CN");
            AddOverlay("COMMANDO_SPECIAL_ALT1_DESCRIPTION", $"扔出一枚手雷，爆炸可造成{CommandoTweak.ThrowGrenadeDamageCoefficient.ToDmgPct()}的伤害。最多可投掷2枚。", "zh-CN");
            AddOverlay("COMMANDO_SPECIAL_DESCRIPTION", $"<style=cIsDamage>眩晕</style>。连续射击，每枚弹丸造成{FireBarrage.damageCoefficient.ToDmgPct()}的伤害。射击次数随攻击速度增加。", "zh-CN");
            ...
            //AddOverlay("ENGI_PRIMARY_DESCRIPTION", $"发射<style=cIsDamage>{BouncingGrenades.maximumGrenadesCount}</style>颗手雷，每颗均可造成{BouncingGrenades.damage.ToDmgPct()}的伤害。", "zh-CN");
            ...
            //AddOverlay("ENGI_SKILL_HARPOON_DESCRIPTION", $"进入<style=cIsUtility>目标标记模式</style>以发射热追踪鱼叉导弹，每发造成{ThermalHarpoons.damage.ToDmgPct()}的伤害。最多可储存{ThermalHarpoons.charges}发。", "zh-CN");
            ...
            //AddOverlay("ENGI_SPIDERMINE_DESCRIPTION", $"放置一枚机器人地雷，在敌人走近时自动引爆，造成{SpiderMines.damage.ToDmgPct()}的伤害，最多放置{SpiderMines.charges}枚。", "zh-CN");
            ...
            AddOverlay("EQUIPMENT_GOLDGAT_DESC", $"发射一阵钱雨，<style=cIsDamage>每颗子弹均造成{GoldGatFire.damageCoefficient.ToDmgPct()}，外加等同于消耗金钱的伤害</style>。每颗子弹消耗{GoldGatFire.baseMoneyCostPerBullet.ToDmg("_枚")}金钱，价格随{"开火时间".ToDmg()}和{"游戏时间".ToUtil()}不断上升。{GoldGatTweak.CostCoefficientPerGoldOnHurt.ToStkPct("（每层成卷的零钱使价格+_）")}。", "zh-CN");
            AddOverlay("EQUIPMENT_LIFESTEALONHIT_DESC", $"击中敌人时<style=cIsHealing>恢复</style>等同于你造成<style=cIsDamage>伤害</style><style=cIsHealing>20%</style>的生命值，并且有{0.1.ToBaseAndStkPct("（每层吸血种子+_）")}的几率使其{流血}，造成{BleedTweak.BleedDamageCoefficient.ToDmgPct("每秒_的基础伤害")}，持续{BleedTweak.BleedDuration.ToUtil("_秒")}；若此次攻击{暴击}，则改为使敌人{出血}，造成{BleedTweak.SurperBleedDamageCoefficient.ToDmgPct("每秒_的基础伤害")}，持续{BleedTweak.SurperBleedDuration.ToUtil("_秒")}{不与其他流血和出血重复}。效果持续<style=cIsHealing>8</style>秒。", "zh-CN");
            AddOverlay("EQUIPMENT_LIFESTEALONHIT_PICKUP", $"击中敌人时有一定几率造成流血或出血，并恢复等同于你造成伤害的一定比例的生命值，持续8秒。", "zh-CN");
            ...
            AddOverlay("EQUIPMENT_TEAMWARCRY_DESC", $"所有友方进入<style=cIsDamage>狂热</style>状态，持续<style=cIsUtility>7</style>秒。<style=cIsUtility>移动速度</style>提升<style=cIsUtility>50%</style>，<style=cIsDamage>攻击速度</style>提升<style=cIsDamage>100%</style>。{"（对于个体而言，每层战争号角可使狂热效果提升50%）".ToStk()}", "zh-CN");
            ...
            AddOverlay("HUNTRESS_PRIMARY_ALT_DESCRIPTION", $"<style=cIsUtility>灵巧</style>。瞄准{HuntressTweak.基础射程.ToUtil("_米") + HuntressTweak.猎人的鱼叉叠加射程.ToStk("（每层猎人的鱼叉+_米）")}内的敌人，拉弓射出<style=cIsDamage>{3}枚</style>跟踪箭，每枚造成{HuntressTweak.FlurryDamageCoefficient.ToDmgPct()}的伤害。如果暴击则发射<style=cIsDamage>{6}</style>枚跟踪箭。", "zh-CN");
            ...
            AddOverlay("HUNTRESS_SECONDARY_DESCRIPTION", $"<style=cIsUtility>灵巧</style>。投掷一把追踪月刃，可弹射最多<style=cIsDamage>{HuntressTweak.LaserGlaiveBounceCount}</style>次，初始造成{HuntressTweak.LaserGlaiveDamageCoefficient.ToDmgPct()}的伤害，每次弹射伤害增加{(HuntressTweak.LaserGlaiveBounceDamageCoefficient - 1f).ToDmgPct()}。", "zh-CN");
            AddOverlay("HUNTRESS_SPECIAL_ALT1_DESCRIPTION", $"向后<style=cIsUtility>传送</style>至空中。最多发射<style=cIsDamage>{HuntressTweak.BallistaBoltCount}</style>道能量闪电，每道造成<style=cIsDamage>{HuntressTweak.BallistaDamageCoefficient.ToDmgPct()}的伤害</style>。", "zh-CN");
            AddOverlay("HUNTRESS_SPECIAL_DESCRIPTION", $"<style=cIsUtility>传送</style>至空中，向目标区域射下箭雨，使区域内所有敌人<style=cIsUtility>减速</style>，并造成每秒{ArrowRain.damageCoefficient.ToDmgPct()}的伤害。", "zh-CN");
            ...
            AddOverlay("ITEM_BLEEDONHIT_DESC", $"命中敌人时有<style=cIsDamage>10%</style><style=cStack>（每层+10%）</style>的几率使敌人{流血}，造成{BleedTweak.BleedDamageCoefficient.ToDmgPct("每秒_的基础伤害")}，持续{BleedTweak.BleedDuration.ToUtil("_秒")}；在造成{流血}的情况下，有{0.02f.ToBaseAndStkPct().ToDmg()}的几率{额外}使敌人{出血}{此处忽略触发系数}，造成{BleedTweak.SurperBleedDamageCoefficient.ToDmgPct("每秒_的基础伤害")}，持续{BleedTweak.SurperBleedDuration.ToUtil("_秒")}{不与其他流血和出血重复}。\n多层{流血}或{出血}将整合为{1.ToDmg()}层。", "zh-CN");
            AddOverlay("ITEM_BLEEDONHITANDEXPLODE_DESC", $"{流血}和{出血}效果造成的持续伤害增加{BleedTweak.BoostDamageMultiplierPerBleedOnHitAndExplode.ToBaseAndStkPct().ToDmg()}。命中时使敌人{流血}，对敌人造成{BleedTweak.BleedDamageCoefficient.ToDmgPct("每秒_的基础伤害")}，持续{BleedTweak.BleedDuration.ToUtil("_秒")}；若此次攻击{暴击}，则改为使敌人{出血}，造成{BleedTweak.SurperBleedDamageCoefficient.ToDmgPct("每秒_的基础伤害")}，持续{BleedTweak.SurperBleedDuration.ToUtil("_秒")}{不与其他流血和出血重复}。<style=cIsDamage>流血或出血</style>的敌人将会在死亡后产生半径{BleedOnHitAndExplodeTweak.BaseRadius.ToBaseWithStk(BleedOnHitAndExplodeTweak.StackRadius).ToDmg("_米") + "（具体范围与敌人体型相关）".ToUtil()}的<style=cIsDamage>鲜血爆炸</style>并造成{BleedOnHitAndExplodeTweak.DamageCoefficient.ToBaseAndStkPct().ToDmg()}的基础伤害。", "zh-CN");
            ...
            AddOverlay("ITEM_DEATHMARK_DESC", $"拥有不少于{"4个".ToDmg()}减益效果的敌人<style=cIsDamage>将被标记为死亡</style>，从所有来源受到的伤害增加{DeathMarkTweak.BaseDamageCoefficient.ToDmgPct() + DeathMarkTweak.StackDamageCoefficient.ToStkPct("（全队每层+_)")}，持续{7.ToBaseAndStk().ToDmg()}秒。<color=#FFFF00>呛鼻毒师特殊效果：对敌人施加Dot减益的持续时间增加40%{"（每层+40%）".ToStk()}。</color>", "zh-CN");
            AddOverlay("ITEM_ELEMENTALRINGVOID_DESC", $"充能完毕后，<style=cIsDamage>不低于400%伤害的攻击</style>击中敌人时会产生一个黑洞，<style=cIsUtility>将15米范围内的敌人吸引至其中心</style>。持续<style=cIsUtility>5</style>秒后坍缩，合计造成{RingsTweak.VoidRingDamageCoefficient.ToBaseAndStkPct().ToDmg()}的总伤害。坍缩时每多一名敌人在黑洞中就提升{"1倍".ToDmg()}的伤害。充能时间<style=cIsUtility>20秒</style><style=cStack>（每层减少10%）</style>。<color=#FFFF00>虚空恶鬼特殊效果：基础充能时间降低至2秒。</color>", "zh-CN");
            ...
            AddOverlay("ITEM_FIRERING_DESC", $"充能完毕后，<style=cIsDamage>不低于{4.ToPct() + 1.ToStkPct("（取双环中层数高的每层+_）")}伤害的攻击</style>击中敌人时会产生一道{"符文火焰龙卷风".ToFire()}轰击敌人，造成{RingsTweak.FireRingDamageCoefficient.ToBaseAndStkPct().ToDmg()}的总持续伤害，同时<style=cIsDamage>点燃</style>敌人。充能时间<style=cIsUtility>10秒</style><style=cStack>（每双手环减少10%）</style>。<color=#FFFF00>工匠特殊效果：基础充能时间降低至1秒。</color>", "zh-CN");
            AddOverlay("ITEM_FLATHEALTH_DESC", $"<style=cIsHealing>最大生命值</style>增加<style=cIsHealing>25</style>点<style=cStack>（每层+25点）</style>。升级获得的<style=cIsHealing>最大生命值</style>增加<style=cIsHealing>20%</style><style=cStack>（每层+20%）</style>。<color=#FFFF00>厨师特殊效果：野牛肉排的效果翻倍。</color>", "zh-CN");
            ...
            AddOverlay("ITEM_HEALONCRIT_DESC", $"获得{HealOnCritTweak.BaseCrit.ToBaseWithStkPct(HealOnCritTweak.StackCrit).ToDmg()}的暴击几率。{"击杀".ToDmg()}敌人后使你恢复{"最大生命值".ToHealing()}的{HealOnCritTweak.HealFraction.ToBaseAndStkPct().ToHealing()}，若{"暴击击杀".ToDmg()}则{"恢复量".ToHealing()}将乘以{"暴击倍率".ToDmg()}。", "zh-CN");
            AddOverlay("ITEM_HEALWHILESAFE_DESC", $"使脱离战斗状态下的<style=cIsHealing>基础生命值再生速度</style>增加{3.ToBaseAndStk().ToHealing("_hp/s")}，外加<style=cIsHealing>基础生命值<color=yellow>百分比</color>再生速度</style>增加{0.016f.ToBaseAndStkPct().ToHealing("_hp/s")}。", "zh-CN");
            AddOverlay("ITEM_ICERING_DESC", $"充能完毕后，<style=cIsDamage>不低于{4.ToPct() + 1.ToStkPct("（取双环中层数高的每层+_）")}伤害的攻击</style>击中敌人时会产生一道{"符文冰霜爆炸".ToIce()}轰击敌人，{"减速".ToIce()}周围{"12米".ToUtil()}内的敌人使其降低<style=cIsUtility>80%</style>的移动速度{RingsTweak.IceRingSlow80BuffDuration.ToBaseAndStk().ToUtil("_秒")}，合计造成{RingsTweak.IceRingDamageCoefficient.ToBaseAndStkPct().ToDmg()}的总伤害。充能时间<style=cIsUtility>10秒</style><style=cStack>（每双手环减少10%）</style>。<color=#FFFF00>工匠特殊效果：基础充能时间降低至1秒。</color>", "zh-CN");
            AddOverlay("ITEM_IGNITEONKILL_DESC", $"击败敌人时产生一次半径<style=cIsDamage>{IgniteOnKillTweak.BaseRadius.ToBaseWithStk(IgniteOnKillTweak.StackRadius)}米{"（具体范围与敌人体型相关）".ToUtil()}</style>的爆炸，造成{IgniteOnKillTweak.ExplosionBaseDamageCoefficient.ToDmgPct("_的基础伤害")}，并<style=cIsDamage>点燃</style>所有被爆炸波及的敌人。<style=cIsDamage>点燃</style>可对敌人造成{IgniteOnKillTweak.IgniteDamageCoefficient.ToBaseAndStkPct().ToDmg("_的基础伤害")}。", "zh-CN");
            ...
            AddOverlay("ITEM_KNURL_DESC", $"{"最大生命值".ToHealing()}增加{40.ToBaseAndStk().ToHealing()}点，外加升级获得的{"最大生命值".ToHealing()}增加{1.6f.ToBaseAndStkPct().ToHealing()}。{"基础生命值再生速度".ToHealing()}增加{1.6f.ToBaseAndStk().ToHealing("_hp/s")}，外加{"基础生命值<color=yellow>百分比</color>再生速度".ToHealing()}增加{0.016f.ToBaseAndStkPct().ToHealing("_hp/s")}。", "zh-CN");
            ...
            AddOverlay("ITEM_MISSILE_DESC", $"有{MissileTweak.BasePercnetChance.ToDmg("_%") + ToStk简单逼近百分百字符串(MissileTweak.半数)}几率向{"导弹发射池".ToUtil()}里添加一枚导弹{MissileTweak.Interval.ToStk("（发射间隔：_秒）")}。每枚合计造成{MissileTweak.DamageCoefficient.ToBaseAndStkPct().ToDmg()}的伤害。", "zh-CN");
            AddOverlay("ITEM_MISSILEVOID_DESC", $"获得一个相当于你最大生命值<style=cIsHealing>{10}%</style>的<style=cIsHealing>护盾</style>。命中敌人时有{"护盾/总护盾".ToHealing()}x{"100%".ToDmg()}几率{此处忽略触发系数}向敌人发射{"1".ToDmg()}发虾米{MissileVoidTweak.Interval.ToStk("（发射间隔：_秒）")}，合计造成{"护盾/总护盾".ToHealing()}x{MissileVoidTweak.DamageCoefficient.ToBaseAndStkPct().ToDmg()}的伤害。<style=cIsVoid>使所有AtG导弹MK.1无效化</style>。", "zh-CN");
            ...
            AddOverlay("ITEM_PARENTEGG_DESC", $"在<style=cIsDamage>受到伤害时</style>回复{ParentEggTweak.HealAmount.ToBaseAndStk().ToHealing("_点")}，外加<style=cIsHealing>受到伤害x{ParentEggTweak.HealFractionFromDamage.ToBaseAndStkPct()}</style>的生命值。", "zh-CN");
            ...
            AddOverlay("MAGE_SECONDARY_ICE_DESCRIPTION", $"<style=cIsUtility>冰冻</style>。使用拥有<style=cIsDamage>穿透</style>效果的纳米枪发动攻击，充能后能造成{CastNanoSpear.minDamage.ToDmgPct()}-{CastNanoSpear.maxDamage.ToDmgPct()}的伤害。爆炸后留下一个<style=cIsUtility>冰冻炸弹</style>，2秒后爆炸并<style=cIsUtility>冰冻</style>附近{"12米".ToUtil()}的敌人，合计造成{"100%".ToDmg()}的伤害。", "zh-CN");
            AddOverlay("MAGE_SECONDARY_LIGHTNING_DESCRIPTION", $"<style=cIsDamage>眩晕</style>。发射一枚纳米炸弹，如果充能将造成{ChargedNanoBomb.minDamage.ToDmgPct()}-{ChargedNanoBomb.maxDamage.ToDmgPct()}的伤害。飞行期间电击周围<style=cIsUtility>{MageTweak.电击半径}</style>米内最多{MageTweak.每次最大电击数.ToDmg("_个")}敌人，每秒合计造成{MageTweak.电击伤害系数.ToDmgPct()}的伤害。", "zh-CN");
            AddOverlay("MAGE_SPECIAL_FIRE_DESCRIPTION", $"<style=cIsDamage>点燃</style>。灼烧面前的所有敌人，对其造成每秒{(Flamethrower.Damage + Flamethrower.Damage / (1f / (Flamethrower.BurnChance / 100f))).ToDmgPct()}的伤害。", "zh-CN");
            ...
            AddOverlay("MERC_SECONDARY_ALT1_DESCRIPTION", $"<style=cIsDamage>一闪</style>。释放一个裂片上勾拳，造成{Uppercut.baseDamageCoefficient.ToDmgPct()}的伤害，并将你送到半空。", "zh-CN");
            ...
            AddOverlay("MERC_SPECIAL_ALT1_DESCRIPTION", $"<style=cIsDamage>一闪</style>。发射一次刀刃之风，最多可对<style=cIsDamage>3</style>名敌人造成<style=cIsDamage>8x{1.ToDmgPct()}的伤害</style>。最后一次打击将<style=cIsUtility>暴露</style>敌人。", "zh-CN");
            AddOverlay("MERC_SPECIAL_DESCRIPTION", $"<style=cIsDamage>一闪</style>。瞄准距离最近的敌人，攻击被瞄准的敌人可对其重复造成{Evis.damageCoefficient.ToDmgPct()}的伤害。<style=cIsUtility>过程中无法被攻击，跳跃键可在击杀时提前退出技能。</style>", "zh-CN");
            ...
            AddOverlay("RAILGUNNER_SNIPE_CRYO_DESCRIPTION", $"<style=cIsUtility>冰冻</style>。发射一枚超低温射弹，造成{RailgunnerTweak.CryochargeDamageCoefficient.ToDmgPct()}的伤害。", "zh-CN");
            AddOverlay("RAILGUNNER_SNIPE_HEAVY_DESCRIPTION", $"发射一枚重型射弹，造成{RailgunnerTweak.M99DamageCoefficient.ToDmgPct()}的伤害。", "zh-CN");
            AddOverlay("RAILGUNNER_SNIPE_LIGHT_DESCRIPTION", $"发射一枚轻型射弹，造成{RailgunnerTweak.HH44DamageCoefficient.ToDmgPct()}的伤害。", "zh-CN");
            AddOverlay("RAILGUNNER_SNIPE_SUPER_DESCRIPTION", $"发射一枚造成{RailgunnerTweak.SuperchargeDamageCoefficient.ToDmgPct()}的伤害且具有{RailgunnerTweak.SuperchargeCritDamageCoefficient.ToDmg()}倍{"暴击伤害".ToDmg()}的超载射弹</style>。", "zh-CN");
            AddOverlay("RAILGUNNER_SPECIAL_ALT_DESCRIPTION", $"<style=cIsUtility>冰冻</style>。发射<style=cIsDamage>具有穿透效果</style>的子弹，造成{RailgunnerTweak.CryochargeDamageCoefficient.ToDmgPct()}的伤害。", "zh-CN");
            AddOverlay("RAILGUNNER_SPECIAL_DESCRIPTION", $"发射一枚<style=cIsDamage>具有穿刺效果，</style>造成{RailgunnerTweak.SuperchargeDamageCoefficient.ToDmgPct()}的伤害且具有{RailgunnerTweak.SuperchargeCritDamageCoefficient.ToDmg()}倍暴击伤害</style>的超载射弹。之后，<style=cIsHealth>你的所有武器都将失灵</style>，持续<style=cIsHealth>{5}</style>秒。", "zh-CN");
            ...
            AddOverlay("SPIKESTRIPSKILL_" + PlasmaCoreSpikestripContent.Content.Skills.Weave.instance.SkillToken + "_DESCRIPTION", $"<style=cIsVoid>纠缠</style>。快速攻击敌人，每次造成<style=cIsDamage>{PlasmaCoreSpikestripContent.Content.Skills.States.FireWeave.damageCoefficient.ToDmgPct()}的伤害</style>。");
            ...
            //AddOverlay("TREEBOT_SPECIAL_ALT1_DESCRIPTION", $"发射弩弹，造成{DIRECTIVEHarvest.damage.ToDmgPct()}的伤害且弩弹将<style=cIsDamage>注入</style>一个敌人。此敌人死亡时，掉落多个<style=cIsHealing>果实</style>，可治疗<style=cIsHealing>{DIRECTIVEHarvest.percentHeal.ToPct()}的生命值</style>{(DIRECTIVEHarvest.giveBuffs ? "，并且给予随机<style=cIsHealing>增益</style>" : "")}。", "zh-CN");
            ...
            AddOverlay("TREEBOT_UTILITY_ALT1_DESCRIPTION", $"{FirePlantSonicBoom.healthCostFraction.ToPct().ToHealth()}生命值。发射一次<style=cIsUtility>音爆</style>并对敌人造成{TreebotTweak.FirePlantSonicBoomDamageCoefficient.ToDmgPct()}的伤害。每命中一个目标便会对你治疗{FirePlantSonicBoom.healthFractionPerHit.ToHealPct()}的生命值。", "zh-CN");
        ...
        private static string Language_GetLocalizedStringByToken(On.RoR2.Language.orig_GetLocalizedStringByToken orig, Language self, string token) {
            ...
            if (self.name == "zh-CN" && result.Length > 0 && !strings.Contains(token) && (大小写字母串.Contains(result.Substring(0, 1)) || (result.StartsWith("<") && 大小写字母串.Contains(result.Substring(result.IndexOf('>') + 1, 1))))) {
                strings.Add(token);
