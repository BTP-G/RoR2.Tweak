...
namespace BtpTweak {
    ...
    public sealed class Localizer : IEventHandlers {
        ...
        public void 基础汉化() {
            ...
            R2API.LanguageAPI.AddOverlay("EQUIPMENT_TEAMWARCRY_DESC", $"所有友方进入<style=cIsDamage>狂热</style>状态，持续<style=cIsUtility>7</style>秒。<style=cIsUtility>移动速度</style>提升<style=cIsUtility>50%</style>，<style=cIsDamage>攻击速度</style>提升<style=cIsDamage>100%</style>。{"（对于个体而言，每层狂战士肩甲可使狂热效果提升50%）".ToStk()}", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("GOLDENKNURL_DESC", $"{"最大生命值".ToHealing()}增加{GoldenCoastPlus.GoldenCoastPlus.KnurlHealth.Value.ToBaseAndStkPct().ToHealing()}，{"生命值再生".ToHealing()}增加{GoldenCoastPlus.GoldenCoastPlus.KnurlRegen.Value.ToHealing() + GoldenCoastPlus.GoldenCoastPlus.KnurlRegen.Value.ToStk("（每层+_）")}hp/s，{"护甲".ToUtil()}增加{GoldenCoastPlus.GoldenCoastPlus.KnurlArmor.Value.ToHealing() + GoldenCoastPlus.GoldenCoastPlus.KnurlArmor.Value.ToStk("（每层+_）")}点。 \n<color=yellow>黄金</color>：{"再生速度".ToHealing()}提升{"50%".ToHealing() + "（每层+50%；每有1,000,000金钱+1%）".ToStk()}。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_BARRIERONOVERHEAL_DESC", $"过量<style=cIsHealing>治疗</style>会使你获得一道<style=cIsHealing>临时屏障</style>，可阻挡相当于{Aegis.BaseOverheal.Value.ToHealPct() + Aegis.StackOverheal.Value.ToStkPct()}已<style=cIsHealing>治疗</style>生命值的伤害。增加{Aegis.BaseArmor.Value.ToHealing() + Aegis.StackArmor.Value.ToStk("（每层+_）")}点护甲，<style=cIsHealing>临时护盾</style>激活时再增加{Aegis.BaseBarrierArmor.Value.ToHealing() + Aegis.StackBarrierArmor.Value.ToStk("（每层+_）")}点护甲。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_BEHEMOTH_DESC", $"你的所有<style=cIsDamage>攻击均会爆炸</style>，对{BrilliantBehemoth.BaseRadius.Value.ToDmg() + BrilliantBehemoth.StackRadius.Value.ToStk("（每层+_）")}米范围内的敌人合计造成{BrilliantBehemoth.BaseDamage.Value.ToDmgPct() + BrilliantBehemoth.StackDamage.Value.ToStkPct()}的额外伤害。<color=#FFFF00>船长特殊效果：关于“火神霰弹枪”（详情看技能介绍）。</color>", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_BONUSGOLDPACKONKILL_DESC", $"<style=cIsDamage>击杀</style>获得的<color=yellow>金钱</color>增加{GhorsTome.BaseMult.Value.ToPct().ToUtil() + GhorsTome.StackMult.Value.ToStkPct()}。\n<style=cIsDamage>击杀敌人</style>时有{GhorsTome.BaseChance.Value.ToUtil("_%") + GhorsTome.StackChance.Value.ToStk("（每层+_%）")}的几率掉落价值<style=cIsUtility>25<color=yellow>金钱</color></style>的宝物<style=cIsUtility>（价值随时间变化）</style>。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_CHAINLIGHTNING_DESC", $"有{ChainLightningTweak.BasePercentChance.ToDmg() + "（概率公式：100%x(层数÷(层数+3))）".ToStk()}的几率发射<style=cIsDamage>连锁闪电</style>{0.2f.ToStk("（发射间隔：_秒）")}，对{ChainLightningTweak.BaseRadius.ToBaseWithStk(ChainLightningTweak.StackRadius).ToUtil()}米内的最多<style=cIsDamage>{ChainLightningTweak.Bounces}个</style>目标合计造成{ChainLightningTweak.BaseDamageCoefficient.ToDmgPct()}的伤害。<color=#FFFF00>工匠特殊效果：关于“充能完毕的纳米炸弹”（详情看技能介绍）。</color>", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_CHAINLIGHTNINGVOID_DESC", $"有{ChainLightningVoidTweak.BasePercentChance.ToDmg() + "（概率公式：100%x(层数÷(层数+3))）".ToStk()}的几率发射<style=cIsDamage>虚空闪电</style>，对同一个敌人合计造成<style=cIsDamage>{ChainLightningVoidTweak.TotalStrikes}x{ChainLightningVoidTweak.BaseDamageCoefficient.ToDmgPct()}</style>的伤害。<style=cIsVoid>使所有尤克里里无效化</style>。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_DEATHMARK_DESC", $"拥有{DeathMark.CountTrigger.Value}个或更多减益效果的敌人<style=cIsDamage>将被标记为死亡</style>，从所有来源受到的伤害增加{DeathMark.BaseDamage.Value.ToDmgPct() + DeathMark.StackDamage.Value.ToPct().ToStk("（全队每层+_)")}，持续{DeathMark.DebuffDuration.Value.ToUtil()}秒。<color=#FFFF00>呛鼻毒师特殊效果：对敌人施加Dot减益的持续时间增加40%{"（每层+40%）".ToStk()}。</color>", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_ENERGIZEDONEQUIPMENTUSE_DESC", $"激活装备可使你获得{WarHorn.BaseBuffAtkSpd.Value.ToDmgPct() + WarHorn.StackBuffAtkSpd.Value.ToStkPct()}攻击速度加成，持续{WarHorn.BaseBuffDuration.Value.ToUtil("_秒") + WarHorn.StackBuffDuration.Value.ToStk("（每层+_秒）")}。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_FIREBALLSONHIT_DESC", $"命中敌人时有{"10%".ToDmg() + "（概率公式：100%x(层数÷(层数+9))）".ToStk()}的几率向敌人的{"岩浆球喷泉".ToFire()}中添加{"1颗岩浆球".ToFire() + ModConfig.喷泉喷射间隔.Value.ToStk("（喷射间隔：_秒）")}，造成{FireballsOnHitTweak.DamageCoefficient.ToBaseAndStkPct().ToDmg()}的伤害，并<style=cIsDamage>点燃</style>所有敌人。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_FIREWORK_DESC", $"命中敌人时有{"5%".ToDmg() + "（每层+5%）".ToStk()}机率向{"烟花发射池".ToUtil()}里添加一枚烟花{ModConfig.导弹发射间隔.Value.ToStk("（发射间隔：_秒）")}。每枚合计造成{FireworkTweak.BaseDamageCoefficent.ToDmgPct()}的伤害。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_HEALONCRIT_DESC", $"获得{0.1f.ToBaseWithStkPct(0.05f).ToDmg()}的暴击几率，<style=cIsDamage>暴击</style>还会使你恢复{8f.ToBaseWithStk(4f).ToHealing()}点{"生命值".ToHealing()}。", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_HEALWHILESAFE_DESC", $"使脱离战斗状态下的<style=cIsHealing>生命值再生</style>增加{CautiousSlug.BaseSafeRegen.Value.ToBaseWithStk(CautiousSlug.StackSafeRegen.Value).ToHealing()}hp/s，外加<style=cIsHealing>生命值<color=yellow>百分比</color>再生</style>增加{CautiousSlug.BaseSafeRegenFraction.Value.ToBaseWithStkPct(CautiousSlug.StackSafeRegenFraction.Value).ToHealing()}hp/s。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_IMMUNETODEBUFF_DESC", $"增加{BensRainCoat.BaseHealthPercent.Value.ToHealPct() + BensRainCoat.StackHealthPercent.Value.ToStkPct()}最大生命值。免疫<style=cIsUtility>{BensRainCoat.BaseImmunity.Value}个<style=cStack>（每层+{BensRainCoat.StackImmunity.Value}个）</style></style><style=cIsDamage>减益效果</style>，并在触发后的<style=cIsUtility>{BensRainCoat.ImmunityDuration.Value}</style>秒内免疫所有<style=cIsDamage>减益</style>。每<style=cIsUtility>{BensRainCoat.BaseRecharge.Value}</style>秒<style=cStack>（每层减少{BensRainCoat.StackReduction.Value.ToPct()}）</style>充能一次</style>。", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_INFUSION_DESC", $"每击败一名敌人后使{"自身外加主人".ToHealing() + "（如果有）".ToStk()}<style=cIsHealing>永久性</style>增加<style=cIsHealing>1点</style><style=cStack>（每层+1点）</style>最大生命值，最多增加<style=cIsHealing>自身等级x0.3x自身基础血量x全队层数</style>点。", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_KNURL_DESC", $"{"最大生命值".ToHealing()}增加{TitanicKnurl.BaseHealth.Value.ToHealing() + TitanicKnurl.StackHealth.Value.ToStk("（每层+_）")}点。{"生命值再生".ToHealing()}增加{TitanicKnurl.BaseRegen.Value.ToHealing() + TitanicKnurl.StackRegen.Value.ToStk("（每层+_）")}hp/s，外加{"生命值<color=yellow>百分比</color>再生".ToHealing()}增加{TitanicKnurl.BaseRegenFraction.Value.ToHealPct() + TitanicKnurl.StackRegenFraction.Value.ToStkPct()}hp/s。", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_LIGHTNINGSTRIKEONHIT_DESC", $"命中敌人时有{"10%".ToDmg() + "（概率公式：100%x(层数÷(层数+9))）".ToStk()}的几率向敌人的{"雷电召唤池".ToLightning()}中添加{"1次雷击".ToLightning() + 0.5f.ToStk("（召唤间隔：_秒）")}，造成<style=cIsDamage>300%</style><style=cStack>（每层+300%）</style>的伤害。<color=#FFFF00>装卸工特殊效果：关于“雷霆拳套”（详情看 权杖 技能介绍）。</color>", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_MISSILE_DESC", $"有{"10%".ToDmg() + "（概率公式：100%x(层数÷(层数+9))）".ToStk()}机率向{"导弹发射池".ToUtil()}里添加一枚导弹{ModConfig.导弹发射间隔.Value.ToStk("（发射间隔：_秒）")}。每枚合计造成{"200%".ToDmg() + "（每层增加200%）".ToStk()}的伤害。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_NEARBYDAMAGEBONUS_DESC", $"对<style=cIsDamage>{FocusCrystal.NearbyRange.Value}米</style>内的敌人伤害增加{FocusCrystal.BaseDamage.Value.ToDmgPct()}<style=cStack>（每层+{FocusCrystal.StackDamage.Value.ToPct()}）</style>.", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_NOVAONHEAL_DESC", $"将{NkuhanasOpinion.BaseStore.Value.ToHealPct()}<style=cStack>（每层增加{NkuhanasOpinion.StackStore.Value.ToPct()}）</style>的治疗量储存为<style=cIsHealing>灵魂能量</style>，最大存储量等同于<style=cIsHealing>最大生命值</style>的{NkuhanasOpinion.BaseCapacity.Value.ToHealPct()}<style=cStack>（每层增加{NkuhanasOpinion.StackCapacity.Value.ToPct()}）</style>。当<style=cIsHealing>灵魂能量</style>达到<style=cIsHealing>最大生命值</style>的<style=cIsHealing>{NkuhanasOpinion.ThresholdFraction.Value.ToPct()}</style>时，自动向半径<style=cIsDamage>{NkuhanasOpinion.BaseRange.Value}</style>米内的敌人<style=cIsDamage>发射一颗头骨</style>，造成相当于<style=cIsDamage>{NkuhanasOpinion.BaseDamage.Value.ToDmgPct()}</style><style=cIsHealing>灵魂能量</style>的<style=cIsDamage>伤害</style>。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_SEED_DESC", $"<style=cIsHealing>恢复</style>等同于你造成<style=cIsDamage>伤害</style>{LeechingSeed.BaseLeech.Value.ToHealPct() + ("（每层+" + LeechingSeed.StackLeech.Value.ToPct() + "）").ToStk()}的生命值。<style=cIsUtility>（所有伤害均生效）</style>\n<style=cStack>吸血计算公式 => Pow([伤害] x 0.01, 0.65) x 2</style>。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_SPRINTARMOR_DESC", $"<style=cIsHealing>护甲</style>增加<style=cIsHealing>{RoseBuckler.BaseArmor.Value}</style>点<style=cStack>（每层+{RoseBuckler.StackArmor.Value}点）</style>，<style=cIsUtility>奔跑时</style>会逐渐获得<style=cIsUtility>移速</style>和<style=cIsHealing>护甲</style>加成，最高增加<style=cIsUtility>{RoseBuckler.BaseMomentumMove.Value.ToPct()}</style><style=cStack>（每层+{RoseBuckler.StackMomentumMove.Value.ToPct()}）</style><style=cIsUtility>移速</style>和<style=cIsHealing>{RoseBuckler.BaseMomentumArmor.Value}</style>点<style=cStack>（每层+{RoseBuckler.StackMomentumArmor.Value}点）</style><style=cIsHealing>护甲</style>。", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_SPRINTOUTOFCOMBAT_DESC", $"{"移动速度".ToUtil()}提高{RedWhip.BaseMove.Value.ToPct().ToUtil() + RedWhip.StackMove.Value.ToPct().ToStk("（每层+_）")}，脱离战斗时会使你的<style=cIsUtility>移动速度</style>提高<style=cIsUtility>{RedWhip.BaseSafeMove.Value.ToPct()}</style><style=cStack>（每层增加{RedWhip.StackSafeMove.Value.ToPct()}）</style>。", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_SPRINTWISP_DESC", $"奔跑时自动向周围{LittleDisciple.BaseRange.Value.ToUtil("_米") + LittleDisciple.StackRange.Value.ToStk("（每层+_米）")}内的敌人发射<style=cIsDamage>追踪幽魂</style>，造成{LittleDisciple.BaseDamage.Value.ToDmgPct() + LittleDisciple.StackDamage.Value.ToPct().ToStk("（每层+_）")}的伤害</style>。奔跑时，每<style=cIsUtility>{LittleDisciple.BaseRecharge.Value}秒</style>{LittleDisciple.StackReduction.Value.ToPct().ToStk("（每层-_）")}发射一次。{(LittleDisciple.ScaleMove.Value ? "发射速率随<style=cIsUtility>移动速度</style>增加。" : "")}", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_STICKYBOMB_DESC", $"命中敌人时有<style=cIsDamage>5%</style><style=cStack>（每层增加5%）</style>的机率向敌人的{"黏弹喷泉".ToDmg()}中添加{"1颗黏弹".ToDmg() + ModConfig.喷泉喷射间隔.Value.ToStk("（喷射间隔：_秒）")}，爆炸时合计造成<style=cIsDamage>100%</style>伤害。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_TPHEALINGNOVA_DESC", $"在传送事件中每<style=cIsHealing>{LeptonDaisy.BaseInterval.Value}</style>秒释放一次<style=cIsHealing>治疗新星</style>，<style=cIsHealing>治疗</style>附近所有友方，使他们恢复{LeptonDaisy.BasePulseHeal.Value.ToHealPct()}<style=cStack>（每层+{LeptonDaisy.StackPulseHeal.Value.ToPct()}）</style>的最大生命值。传送器范围内的友方<style=cIsHealing>生命值再生</style>增加<style=cIsHealing>{LeptonDaisy.BaseHoldoutRegen.Value}hp/s</style><style=cStack>（每层+{LeptonDaisy.StackHoldoutRegen.Value}hp/s）</style>，外加<style=cIsHealing>生命值<color=yellow>百分比</color>再生</style>增加<style=cIsHealing>{LeptonDaisy.BaseHoldoutRegenFraction.Value.ToHealPct()}hp/s</style><style=cStack>（每层+{LeptonDaisy.StackHoldoutRegenFraction.Value.ToPct()}hp/s）</style>。<color=#FFFF00>雷克斯特殊效果：治疗效果增加20%{"（每层+20%）".ToStk()}。</color>", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_WARCRYONMULTIKILL_DESC", $"<style=cIsDamage>击杀敌人</style>会使你获得<style=cIsDamage>狂热</style>增益，持续<style=cIsDamage>{BerzerkersPauldron.BaseBuffDuration.Value}秒</style>。<style=cIsDamage>狂热</style>使<style=cIsUtility>移动速度</style>提高<style=cIsUtility>{BerzerkersPauldron.BuffMove.Value.ToPct()}</style>，最高<style=cIsUtility>{(BerzerkersPauldron.BuffMove.Value * BerzerkersPauldron.BaseBuffCount.Value).ToPct()}<style=cStack>（每层+{(BerzerkersPauldron.BuffMove.Value * BerzerkersPauldron.StackBuffCount.Value).ToPct()}）</style></style>，使<style=cIsDamage>攻击速度</style>提高<style=cIsDamage>{BerzerkersPauldron.BuffAtkSpd.Value.ToPct()}</style>，最高<style=cIsUtility>{(BerzerkersPauldron.BuffAtkSpd.Value * BerzerkersPauldron.BaseBuffCount.Value).ToPct()}<style=cStack>（每层+{(BerzerkersPauldron.BuffAtkSpd.Value * BerzerkersPauldron.StackBuffCount.Value).ToPct()}）</style></style>。", "zh-CN");
        ...
        private void MainMenuController_Start(On.RoR2.UI.MainMenu.MainMenuController.orig_Start orig, RoR2.UI.MainMenu.MainMenuController self) {
            orig(self);
            On.RoR2.UI.MainMenu.MainMenuController.Start -= MainMenuController_Start;
            TPDespair.ZetAspects.Language.tokens["zh-CN"]["ITEM_SHIELDONLY_DESC"] += $"\n<style=cIsLunar>完美的象征：护盾百分比再生速度增加{TPDespair.ZetAspects.Configuration.AspectLunarRegen.Value}%{TPDespair.ZetAspects.Configuration.AspectLunarRegen.Value.ToStk("（每层+_%）")}。</style>";
        }
    }
}