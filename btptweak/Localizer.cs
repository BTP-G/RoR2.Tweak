...
namespace BtpTweak {
    ...
    internal static class Localizer {
        ...
        private static void 基础汉化() {
            ...
            AddOverlay("CAPTAIN_UTILITY_ALT1_DESCRIPTION", $"<style=cIsDamage>眩晕</style>。{"消耗4层充能".ToUtil()}向<style=cIsDamage>UES顺风号</style>请求发动一次<style=cIsDamage>动能打击</style>。在<style=cIsUtility>20秒</style>后，对所有角色造成{"50000%".ToDmg()}的伤害。", "zh-CN");
            ...
            AddOverlay("COMMANDO_SECONDARY_DESCRIPTION", $"发射一枚<style=cIsDamage>穿甲弹</style>，造成{PhaseRound.Damage.ToDmgPct()}的伤害。每次穿透敌人，造成的伤害提高<style=cIsDamage>40%</style>。", "zh-CN");
            AddOverlay("COMMANDO_SPECIAL_ALT1_DESCRIPTION", $"{"点燃".ToDmg()}。扔出一枚手雷，爆炸可造成{FragGrenade.Damage.ToDmgPct()}的伤害。最多可投掷2枚。", "zh-CN");
            AddOverlay("COMMANDO_SPECIAL_DESCRIPTION", $"<style=cIsDamage>眩晕</style>。连续射击，每枚弹丸造成{SuppressiveFire.Damage.ToDmgPct()}的伤害。射击次数随攻击速度增加。", "zh-CN");
            ...
            AddOverlay("ENGI_UTILITY_DESCRIPTION", $"放置一个<style=cIsUtility>无法穿透且有击退力的护盾</style>来阻挡弹幕，并且可以击退小型敌人{((BubbleShield.damage > 0f) ? "，击退时造成" + (BubbleShield.damage / BubbleShield.ticks).ToDmgPct() + "的伤害" : "")}。每个盾需要消耗{BubbleShield.chargesToConsume}层充能，存在<style=cIsUtility>{BubbleShield.duration}秒</style>。", "zh-CN");
            ...
            AddOverlay("EQUIPMENT_GOLDGAT_DESC", $"发射一阵钱雨，<style=cIsDamage>每颗子弹均造成100%{"（每层成卷的零钱+100%）".ToStk()}，外加等同于消耗金钱的伤害</style>。每颗子弹消耗1枚{"（启动后，每秒+1枚）".ToStk()}金钱，价格随游戏时间不断上升。", "zh-CN");
            AddOverlay("EQUIPMENT_LIFESTEALONHIT_DESC", $"击中敌人时有{"10%".ToDmg() + "（每层吸血种子+10%）".ToStk()}的几率造成流血{"（暴击时造成小出血）".ToDmg() + "（不与其他流血和出血重复）".ToUtil()}，并<style=cIsHealing>恢复</style>等同于你造成<style=cIsDamage>伤害</style><style=cIsHealing>20%</style>的生命值。效果持续<style=cIsHealing>8</style>秒。", "zh-CN");
            AddOverlay("EQUIPMENT_LIFESTEALONHIT_PICKUP", $"击中敌人时有10%的概率造成流血或出血，并恢复等同于你造成伤害的一定比例的生命值，持续8秒。", "zh-CN");
            ...
            AddOverlay("HAT_MAGE_UTILITY_FIRE_DESCRIPTION", $"<style=cIsUtility>灵巧</style>。<style=cIsDamage>点燃</style>。向前冲刺，在身后召唤每秒造成{HIFUArtificerTweaks.Main.flamewallDamage.Value.ToDmgPct()}的伤害的火柱</style>。", "zh-CN");
            AddOverlay("HAT_MAGE_UTILITY_FIRE_NAME", "火墙", "zh-CN");
            AddOverlay("HIDDENGOLDBUFFITEM_NAME", "奥利雷奥尼特的祝福", "zh-CN");
            AddOverlay("HUNTRESS_PRIMARY_ALT_DESCRIPTION", $"<style=cIsUtility>灵巧</style>。瞄准{HuntressTweak.基础射程.ToUtil("_米") + HuntressTweak.猎人的鱼叉叠加射程.ToStk("（每层猎人的鱼叉+_米）")}内的敌人，拉弓射出<style=cIsDamage>{Flurry.minArrows}枚</style>跟踪箭，每枚造成{Flurry.damage.ToDmgPct()}的伤害。如果暴击则发射<style=cIsDamage>{Flurry.maxArrows}</style>枚跟踪箭。", "zh-CN");
            AddOverlay("HUNTRESS_PRIMARY_DESCRIPTION", $"<style=cIsUtility>灵巧</style>。瞄准{HuntressTweak.基础射程.ToUtil("_米") + HuntressTweak.猎人的鱼叉叠加射程.ToStk("（每层猎人的鱼叉+_米）")}内的敌人，快速射出一枚能够造成{Strafe.damage.ToDmgPct()}伤害的跟踪箭。", "zh-CN");
            AddOverlay("HUNTRESS_SECONDARY_DESCRIPTION", $"{(LaserGlaive.agile ? "<style=cIsUtility>灵巧</style>。" : "")}投掷一把追踪月刃，可弹射最多<style=cIsDamage>{LaserGlaive.bounceCount}</style>次，初始造成{LaserGlaive.damage.ToDmgPct()}的伤害，每次弹射伤害增加<style=cIsDamage>{Math.Round((double)((LaserGlaive.bounceDamage - 1f) * 100f), 1)}%</style>。", "zh-CN");
            AddOverlay("HUNTRESS_SPECIAL_ALT1_DESCRIPTION", $"向后<style=cIsUtility>传送</style>至空中。最多发射<style=cIsDamage>{Ballista.boltCount}</style>道能量闪电，造成<style=cIsDamage>{Ballista.boltCount}x{Ballista.damage.ToDmgPct()}的伤害</style>。", "zh-CN");
            AddOverlay("HUNTRESS_SPECIAL_DESCRIPTION", $"<style=cIsUtility>传送</style>至空中，向目标区域射下箭雨，使区域内所有敌人<style=cIsUtility>减速</style>，并造成每秒{(3f * ArrowRain.damage).ToDmgPct()}的伤害。", "zh-CN");
            ...
            AddOverlay("ITEM_BLEEDONHIT_DESC", $"攻击有<style=cIsDamage>10%</style><style=cStack>（每层增加10%）</style>的几率使敌人<style=cIsDamage>流血</style>{"（不与其他流血重复）".ToUtil()}，造成<style=cIsDamage>320%</style>的基础伤害。每<style=cIsDamage>2</style>层<style=cIsDamage>流血或出血</style>将整合为{1.ToDmg()}层。", "zh-CN");
            ...
            AddOverlay("ITEM_PARENTEGG_DESC", $"在<style=cIsDamage>受到伤害时</style>回复<style=cIsHealing>20点</style>，外加<style=cIsHealing>受到伤害x1%<style=cStack>（每层+1%）</style></style>的生命值。", "zh-CN");
            ...
            AddOverlay("ITEM_SEED_DESC", $"击中敌人时<style=cIsHealing>恢复</style>{1.ToBaseAndStk().ToHealing("_点")}生命值，外加造成伤害时{"恢复".ToHealing()}等同于你造成<style=cIsDamage>伤害</style>{SeedTweak.Leech.ToBaseAndStkPct().ToHealing()}的生命值。\n<style=cStack>吸血(计算公式) = Sqrt([伤害] x {SeedTweak.Leech.ToPct()})（[伤害] x {SeedTweak.Leech.ToPct()}开平方根）</style>", "zh-CN");
            ...
            AddOverlay("ITEM_SLOWONHITVOID_DESC", $"命中敌人时有<style=cIsUtility>5%</style><style=cStack>（每层+5%）</style>几率使敌人<style=cIsDamage>定身</style><style=cIsUtility>1秒</style><style=cStack>（每层+1秒）</style>，但是对<style=cIsVoid>虚空生物</style><style=cDeath>无效</style>。<style=cIsVoid>使所有时空装置无效化</style>。", "zh-CN");
            ...
            AddOverlay("LOADER_SPECIAL_ALT_DESCRIPTION", $"<style=cIsUtility>超重</style>。用重拳砸向地面，造成{(Thunderslam.damage + Thunderslam.yVelocityCoeff * 25f).ToDmgPct()}的范围伤害。", "zh-CN");
            AddOverlay("LOADER_SPECIAL_DESCRIPTION", $"扔出飘浮电塔，可<style=cIsDamage>电击</style>周围{M551Pylon.aoe.ToUtil()}米{M551Pylon.aoe.ToStk("（每层不稳定的特斯拉线圈+_米）")}内最多<style=cIsDamage>3</style>名敌人，电流最多可弹射{M551Pylon.bounces}次{"（每层不稳定的特斯拉线圈+1次）".ToStk()}，造成{M551Pylon.damage.ToDmgPct()}的伤害。可被<style=cIsUtility>格挡</style>。", "zh-CN");
            AddOverlay("LOADER_UTILITY_ALT1_DESCRIPTION", $"<style=cIsUtility>重型</style>。发动一次<style=cIsUtility>单体攻击</style>直拳，造成{"2100%的伤害".ToDmg()}，<style=cIsDamage>震荡</style>锥形区域内的所有敌人并造成<style=cIsDamage>1000%的伤害</style>。", "zh-CN");
            AddOverlay("LOADER_UTILITY_DESCRIPTION", $"<style=cIsUtility>重型</style>。发动一次<style=cIsUtility>穿透</style>直拳，造成{"600%-2700%的伤害".ToDmg()}。", "zh-CN");
            ...
            AddOverlay("MERC_SECONDARY_ALT1_DESCRIPTION", $"<style=cIsDamage>一闪</style>。释放一个裂片上勾拳，造成{"600%".ToDmg()}的伤害，并将你送到半空。", "zh-CN");
            ...
            AddOverlay("MERC_SPECIAL_ALT1_DESCRIPTION", $"<style=cIsDamage>一闪</style>。发射一次刀刃之风，最多可对<style=cIsDamage>3</style>名敌人造成<style=cIsDamage>8x{SlicingWinds.damage.ToDmgPct()}的伤害</style>。最后一次打击将<style=cIsUtility>暴露</style>敌人。", "zh-CN");
            AddOverlay("MERC_SPECIAL_DESCRIPTION", $"<style=cIsDamage>一闪</style>。瞄准距离最近的敌人，攻击被瞄准的敌人可对其重复造成{Eviscerate.damageCoefficient.ToDmgPct()}的伤害。<style=cIsUtility>过程中无法被攻击，跳跃键可在击杀时提前退出技能。</style>", "zh-CN");
            ...
            AddOverlay("RAILGUNNER_SECONDARY_ALT_DESCRIPTION", $"{"手动上弹".ToUtil()}。启动你的<style=cIsUtility>近程瞄准镜</style>，高亮显示<style=cIsDamage>弱点</style>，并将你的武器转化为一门可造成{HH44Marksman.Damage.ToDmgPct()}伤害的快速磁轨炮。", "zh-CN");
            AddOverlay("RAILGUNNER_SECONDARY_DESCRIPTION", $"{"主动上弹".ToUtil()}。启动你的<style=cIsUtility>远程瞄准镜</style>，高亮显示<style=cIsDamage>弱点</style>，并将你的武器转化为一门可造成{M99Sniper.Damage.ToDmgPct()}伤害的穿刺磁轨炮。", "zh-CN");
            AddOverlay("RAILGUNNER_SNIPE_CRYO_DESCRIPTION", $"<style=cIsUtility>冰冻</style>。发射一枚超低温射弹，造成{Cryocharge.Damage.ToDmgPct()}的伤害。", "zh-CN");
            AddOverlay("RAILGUNNER_SNIPE_HEAVY_DESCRIPTION", $"发射一枚重型射弹，造成{M99Sniper.Damage.ToDmgPct()}的伤害。", "zh-CN");
            AddOverlay("RAILGUNNER_SNIPE_LIGHT_DESCRIPTION", $"发射一枚轻型射弹，造成{HH44Marksman.Damage.ToDmgPct()}的伤害。", "zh-CN");
            AddOverlay("RAILGUNNER_SNIPE_SUPER_DESCRIPTION", $"发射一枚造成{Supercharge.Damage.ToDmgPct()}的伤害且具有{Supercharge.CritDamage.ToDmg()}倍{"暴击伤害".ToDmg()}的超载射弹</style>。", "zh-CN");
            AddOverlay("RAILGUNNER_SPECIAL_ALT_DESCRIPTION", $"<style=cIsUtility>冰冻</style>。发射<style=cIsDamage>具有穿透效果</style>的子弹，造成{Cryocharge.Damage.ToDmgPct()}的伤害。", "zh-CN");
            AddOverlay("RAILGUNNER_SPECIAL_DESCRIPTION", $"发射一枚<style=cIsDamage>具有穿刺效果，</style>造成{Supercharge.Damage.ToDmgPct()}的伤害且具有{Supercharge.CritDamage.ToDmg()}倍暴击伤害</style>的超载射弹。之后，<style=cIsHealth>你的所有武器都将失灵</style>，持续<style=cIsHealth>{Supercharge.HopooBalance}</style>秒。", "zh-CN");
            AddOverlay("RAILGUNNER_UTILITY_ALT_DESCRIPTION", $"扔出一部装置，该装置可使附近的{"敌人减速80%，射弹减速99%".ToUtil()}，友方提速{PolarFieldDevice.SpeedBuffVal.ToPct().ToUtil()}", "zh-CN");
            ...
            AddOverlay("TREEBOT_UTILITY_ALT1_DESCRIPTION", $"{BrambleVolley.healthCost.ToPct().ToHealth()}生命值。发射一次<style=cIsUtility>音爆</style>并对敌人造成{BrambleVolley.damage.ToDmgPct()}的伤害。每命中一个目标便会对你治疗{BrambleVolley.healPercent.ToHealPct()}的生命值。", "zh-CN");
        ...
        private static void 权杖技能汉化() {
            ...
            AddOverlay("ANCIENTSCEPTER_COMMANDO_BARRAGEDESC", Language.GetString("COMMANDO_SPECIAL_DESCRIPTION") + $"射击速度提高100%{"（每层+100%）".ToStk()}，额外发射{"1发".ToDmg()}子弹。按住你的主要技能可以更精确地射击。".ToScepterDesc(), "zh-CN");
