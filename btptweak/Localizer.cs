...
namespace BtpTweak {
    ...
    internal static class Localizer {
        ...
        private static void 基础汉化() {
            ...
            AddOverlay("CAPTAIN_UTILITY_ALT1_DESCRIPTION", $"<style=cIsDamage>眩晕</style>。{"消耗4层充能".ToUtil()}向<style=cIsDamage>UES顺风号</style>请求发动一次<style=cIsDamage>动能打击</style>。在<style=cIsUtility>20秒</style>后，对所有角色造成{CaptainTweak.CallAirstrikeAltDamageCoefficient.ToDmg()}的伤害。", "zh-CN");
            ...
            AddOverlay("ITEM_ELEMENTALRINGVOID_DESC", $"充能完毕后，<style=cIsDamage>不低于400%伤害的攻击</style>击中敌人时会产生一个黑洞，<style=cIsUtility>将15米范围内的敌人吸引至其中心</style>。持续<style=cIsUtility>5</style>秒后坍缩，合计造成{RingsTweak.VoidRingDamageCoefficient.ToBaseAndStkPct().ToDmg()}的伤害。坍缩时每多一名敌人在黑洞中就提升{"1倍".ToDmg()}的伤害。充能时间<style=cIsUtility>20秒</style><style=cStack>（每层减少10%）</style>。<color=#FFFF00>虚空恶鬼特殊效果：基础充能时间降低至2秒。</color>", "zh-CN");
            ...
            AddOverlay("LOADER_SPECIAL_ALT_DESCRIPTION", $"<style=cIsUtility>重型</style>。{"眩晕".ToDmg()}。用重拳砸向地面，造成{LoaderTweak.GroundSlamBaseDamageCoefficient.ToDmgPct()}的范围伤害。", "zh-CN");
            AddOverlay("LOADER_SPECIAL_DESCRIPTION", $"扔出飘浮电塔，可<style=cIsDamage>电击</style>周围{LoaderTweak.PylonRange.ToUtil()}米{LoaderTweak.PylonRange.ToStkPct("（每层不稳定的特斯拉线圈+_米）")}内最多<style=cIsDamage>{LoaderTweak.PylonFireCount}</style>名{LoaderTweak.PylonFireCount.ToStk("（每层不稳定的特斯拉线圈+_名）")}敌人，电流最多可弹射{LoaderTweak.PylonBounces}次{LoaderTweak.PylonBounces.ToStk("（每层不稳定的特斯拉线圈+_次）")}，造成{LoaderTweak.PylonDamageCoefficient.ToDmgPct()}的伤害。可被<style=cIsUtility>格挡</style>。", "zh-CN");
