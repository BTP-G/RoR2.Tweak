...
namespace BtpTweak {
    ...
    public sealed class Localizer : IEventHandlers {
        ...
        public void 基础汉化() {
            ...
            R2API.LanguageAPI.AddOverlay("CAPTAIN_SUPPLY_HEAL_DESCRIPTION", $"每秒为附近10米{"（每级+1米）".ToStack()}所有友方<style=cIsHealing>恢复</style>等同于各个角色<style=cIsHealing>最大生命值</style>{"10%".ToHealing() + "（每级+1%）".ToStack()}的生命值。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("CROCO_SECONDARY_ALT_DESCRIPTION", $"<style=cIsHealing>毒化</style>。<style=cIsDamage>屠杀者</style>。<style=cIsHealing>自我恢复</style>。啃咬一名敌人并造成<style=cIsDamage>320%的伤害</style><style=cStack>（每层怪物牙齿+100%击退）</style>。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("EQUIPMENT_LIFESTEALONHIT_DESC", $"击中敌人时有{"10%".ToDmg() + "（每层吸血种子+10%）".ToStack()}的几率造成流血{"（暴击时造成小出血）".ToDmg() + "（不与其他流血和出血重复）".ToUtil()}，并<style=cIsHealing>恢复</style>等同于你造成<style=cIsDamage>伤害</style><style=cIsHealing>20%</style>的生命值。效果持续<style=cIsHealing>8</style>秒。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("EQUIPMENT_MOLOTOV_DESC", $"投掷<style=cIsDamage>6</style>个燃烧瓶，碎裂时<style=cIsDamage>点燃</style>敌人，造成<style=cIsDamage>500%的基础伤害</style>。每个燃烧瓶都能制造一片燃烧区域，范围随时间扩大，<style=cIsDamage>每秒造成200%的伤害</style>{"（每层汽油使范围和持续时间增加50%）".ToStack()}。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("EQUIPMENT_TEAMWARCRY_DESC", $"所有友方进入<style=cIsDamage>狂热</style>状态，持续<style=cIsUtility>7</style>秒。<style=cIsUtility>移动速度</style>提升<style=cIsUtility>50%</style>，<style=cIsDamage>攻击速度</style>提升<style=cIsDamage>100%</style>。{"（对于个体而言，每层狂战士肩甲可使狂热效果提升50%）".ToStack()}", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("GOLDENKNURL_DESC", $"{"最大生命值".ToHealing()}增加{GoldenCoastPlus.GoldenCoastPlus.KnurlHealth.Value.ToPctWithStack().ToHealing()}，{"生命值再生".ToHealing()}增加{GoldenCoastPlus.GoldenCoastPlus.KnurlRegen.Value.ToHealing() + GoldenCoastPlus.GoldenCoastPlus.KnurlRegen.Value.ToStack("（每层+_）")}hp/s，{"护甲".ToUtil()}增加{GoldenCoastPlus.GoldenCoastPlus.KnurlArmor.Value.ToHealing() + GoldenCoastPlus.GoldenCoastPlus.KnurlArmor.Value.ToStack("（每层+_）")}点。 \n<color=yellow>黄金</color>：{"再生速度".ToHealing()}提升{"50%".ToHealing() + "（每层+50%；每有1,000,000金钱+1%）".ToStack()}。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("HUNTRESS_PRIMARY_ALT_DESCRIPTION", $"<style=cIsUtility>灵巧</style>。瞄准60米<style=cStack>（每层猎人的鱼叉+10米）</style>内的敌人，拉弓射出<style=cIsDamage>{Flurry.minArrows}枚</style>跟踪箭，每枚造成{Flurry.damage.ToDmgPct()}的伤害。如果暴击则发射<style=cIsDamage>{Flurry.maxArrows}</style>枚跟踪箭。", "zh-CN");
            R2API.LanguageAPI.AddOverlay("HUNTRESS_PRIMARY_DESCRIPTION", $"<style=cIsUtility>灵巧</style>。瞄准60米<style=cStack>（每层猎人的鱼叉+10米）</style>内的敌人，快速射出一枚能够造成{Strafe.damage.ToDmgPct()}伤害的跟踪箭。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_ALIENHEAD_DESC", $"使技能{"冷却时间".ToUtil()}减少{AlienHead.BaseCooldown.Value.ToPercent().ToUtil() + AlienHead.StackCooldown.Value.ToStackPct()}。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_BARRIERONOVERHEAL_DESC", $"过量<style=cIsHealing>治疗</style>会使你获得一道<style=cIsHealing>临时屏障</style>，可阻挡相当于{Aegis.BaseOverheal.Value.ToHealPct() + Aegis.StackOverheal.Value.ToStackPct()}已<style=cIsHealing>治疗</style>生命值的伤害。增加{Aegis.BaseArmor.Value.ToHealing() + Aegis.StackArmor.Value.ToStack("（每层+_）")}点护甲，<style=cIsHealing>临时护盾</style>激活时再增加{Aegis.BaseBarrierArmor.Value.ToHealing() + Aegis.StackBarrierArmor.Value.ToStack("（每层+_）")}点护甲。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_BEHEMOTH_DESC", $"你的所有<style=cIsDamage>攻击均会爆炸</style>，对{BrilliantBehemoth.BaseRadius.Value.ToDmg() + BrilliantBehemoth.StackRadius.Value.ToStack("（每层+_）")}米范围内的敌人合计造成{BrilliantBehemoth.BaseDamage.Value.ToDmgPct() + BrilliantBehemoth.StackDamage.Value.ToStackPct()}的额外伤害。<color=#FFFF00>船长特殊效果：关于“火神霰弹枪”（详情看技能介绍）。</color>", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_BONUSGOLDPACKONKILL_DESC", $"<style=cIsDamage>击杀</style>获得的<color=yellow>金钱</color>增加{GhorsTome.BaseMult.Value.ToPercent().ToUtil() + GhorsTome.StackMult.Value.ToStackPct()}。\n<style=cIsDamage>击杀敌人</style>时有{(GhorsTome.BaseChance.Value + "%").ToUtil() + GhorsTome.StackChance.Value.ToStack("（每层+_%）")}的几率掉落价值<style=cIsUtility>25<color=yellow>金钱</color></style>的宝物<style=cIsUtility>（价值随时间变化）</style>。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_BOUNCENEARBY_DESC", $"命中敌人时有<style=cIsDamage>33%</style><style=cStack>（每层增加16.5%）</style>的几率向周围{"33米".ToUtil()}内最多<style=cIsDamage>6名</style>敌人发射<style=cIsDamage>追踪钩爪</style>{0.33f.ToStack("（发射间隔：_秒）")}，合计造成<style=cIsDamage>100%的伤害</style>。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_CHAINLIGHTNING_DESC", $"有{"25%".ToDmg() + "（概率公式：100%x(层数÷(层数+3))）".ToStack()}的几率发射<style=cIsDamage>连锁闪电</style>{0.2f.ToStack("（发射间隔：_秒）")}，对<style=cIsDamage>18米</style><style=cStack>（每层增加3米）</style>内的最多<style=cIsDamage>3个</style>{"（每层+1个）".ToStack()}目标合计造成<style=cIsDamage>60%</style>的伤害。<color=#FFFF00>工匠特殊效果：关于“充能完毕的纳米炸弹”（详情看技能介绍）。</color>", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_CHAINLIGHTNINGVOID_DESC", $"有{"25%".ToDmg() + "（概率公式：100%x(层数÷(层数+3))）".ToStack()}的几率发射<style=cIsDamage>虚空闪电</style>，对同一个敌人合计造成<style=cIsDamage>3{"（每层+1）".ToStack()}x40%</style>的伤害。<style=cIsVoid>使所有尤克里里无效化</style>。", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_CLOVER_DESC", $"所有随机效果的概率提升<style=cIsUtility>25%</style>{"（叠加公式：幸运提升概率 = 原概率x(运气/(|运气|+3))）".ToStack()}。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_CRITGLASSESVOID_DESC", $"你的攻击有<style=cIsDamage>1%</style><style=cStack>（每层+1%）</style>几率对<style=cIsDamage>非Boss敌人</style>造成<style=cIsDamage>生命诅咒</style>，降低<style=cIsHealing>最大生命值</style>。<style=cIsVoid>使所有透镜制作者的眼镜无效化</style>。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_DEATHMARK_DESC", $"拥有{DeathMark.CountTrigger.Value}个或更多减益效果的敌人<style=cIsDamage>将被标记为死亡</style>，从所有来源受到的伤害增加{DeathMark.BaseDamage.Value.ToDmgPct() + DeathMark.StackDamage.Value.ToStackPct("（全队每层+_%)")}，持续{DeathMark.DebuffDuration.Value.ToUtil()}秒。<color=#FFFF00>呛鼻毒师特殊效果：对敌人施加Dot减益的持续时间增加40%{"（每层+40%）".ToStack()}。</color>", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_ELEMENTALRINGVOID_DESC", $"充能完毕后，<style=cIsDamage>不低于400%伤害的攻击</style>击中敌人时会产生一个黑洞，<style=cIsUtility>将15米范围内的敌人吸引至其中心</style>。持续<style=cIsUtility>5</style>秒后坍缩，合计造成{RingsTweak.VoidRingDamageCoefficient.ToPctWithStack().ToDmg()}的伤害。坍缩时每多一名敌人在黑洞中就提升{"100%".ToDmg()}的伤害。充能时间<style=cIsUtility>20</style>秒<style=cStack>（每层减少1秒。至少2秒）</style>。<color=#FFFF00>虚空恶鬼特殊效果：充能时间降低至2秒。</color>", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_ENERGIZEDONEQUIPMENTUSE_DESC", $"激活装备可使你获得{WarHorn.BaseBuffAtkSpd.Value.ToDmgPct() + WarHorn.StackBuffAtkSpd.Value.ToStackPct()}攻击速度加成，持续{WarHorn.BaseBuffDuration.Value.ToUtil("_秒") + WarHorn.StackBuffDuration.Value.ToStack("（每层+_秒）")}。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_EXPLODEONDEATH_DESC", $"击败敌人后召唤一道<style=cIsDamage>岩浆柱</style>，并在{"0.5秒".ToUtil()}后产生半径<style=cIsDamage>12米</style>{"（具体范围与敌人体型相关）".ToUtil()}的爆炸，造成<style=cIsDamage>300%{"（每层+300%）".ToStack()}</style>的基础伤害。", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_EXPLODEONDEATHVOID_DESC", $"对敌人造成{"第一次".ToUtil()}伤害时将<style=cIsDamage>引爆</style>它们，产生半径<style=cIsDamage>10米</style>{"（具体范围与敌人体型相关）".ToUtil()}的爆炸，合计造成<style=cIsDamage>50%x{"敌方剩余百分比生命值".ToHealth()}</style>的总伤害外加{"100%".ToDmg() + "（每层+100%）".ToStack()}的基础伤害，并且<style=cIsDamage>点燃</style>该敌人，但此燃烧伤害{"不享受".ToUtil()}点火罐加成，燃烧总共造成等同于敌人<style=cIsDamage>剩余生命值</style>的伤害。<style=cIsVoid>使所有鬼火无效化</style>。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_FIREBALLSONHIT_DESC", $"命中敌人时有{"10%".ToDmg() + "（概率公式：100%x(层数÷(层数+9))）".ToStack()}的几率向敌人的{"岩浆球喷泉".ToFire()}中添加{"1颗岩浆球".ToFire() + ModConfig.喷泉喷射间隔.Value.ToStack("（喷射间隔：_秒）")}，造成<style=cIsDamage>300%</style><style=cStack>（每层+300%）</style>的伤害，并<style=cIsDamage>点燃</style>所有敌人。", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_FIRERING_DESC", $"充能完毕后，<style=cIsDamage>不低于400%伤害的攻击</style>击中敌人时会产生一道{"符文火焰龙卷风".ToFire()}轰击敌人，造成{RingsTweak.FireRingDamageCoefficient.ToPctWithStack().ToDmg()}的总持续伤害，同时<style=cIsDamage>点燃</style>敌人。充能时间<style=cIsUtility>10</style>秒<style=cStack>（每双手环减少1秒。至少1秒）</style>。<color=#FFFF00>工匠特殊效果：充能时间降低至1秒。</color>", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_FIREWORK_DESC", $"命中敌人时有{"5%".ToDmg() + "（每层+5%）".ToStack()}机率向{"烟花发射池".ToUtil()}里添加一枚烟花{ModConfig.导弹发射间隔.Value.ToStack("（发射间隔：_秒）")}。每枚合计造成{FireworkTweak.BaseDamageCoefficent.ToDmgPct()}的伤害。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_HEALONCRIT_DESC", $"获得{HarvestersScythe.BaseCrit.Value.ToStdWithStack(HarvestersScythe.StackCrit.Value).ToDmg()}%的暴击几率，<style=cIsDamage>暴击</style>还会使你恢复{HarvestersScythe.BaseCritHeal.Value.ToStdWithStack(HarvestersScythe.StackCritHeal.Value).ToHealing()}点{"生命值".ToHealing()}。", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_HEALWHILESAFE_DESC", $"使脱离战斗状态下的<style=cIsHealing>生命值再生</style>增加{CautiousSlug.BaseSafeRegen.Value.ToStdWithStack(CautiousSlug.StackSafeRegen.Value).ToHealing()}hp/s，外加<style=cIsHealing>生命值<color=yellow>百分比</color>再生</style>增加{CautiousSlug.BaseSafeRegenFraction.Value.ToStdPctWithStack(CautiousSlug.StackSafeRegenFraction.Value).ToHealing()}hp/s。", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_ICERING_DESC", $"充能完毕后，<style=cIsDamage>不低于400%伤害的攻击</style>击中敌人时会产生一道{"符文冰霜爆炸".ToIce()}轰击敌人，{"冰冻".ToIce()}周围{"12米".ToUtil()}内的敌人{"2秒".ToUtil()}并使其降低<style=cIsUtility>80%</style>的移动速度<style=cIsUtility>{RingsTweak.IceRingSlow80BuffDuration}秒</style><style=cStack>（每层+{RingsTweak.IceRingSlow80BuffDuration}秒）</style>，合计造成{RingsTweak.IceRingDamageCoefficient.ToPctWithStack().ToDmg()}的总伤害。充能时间<style=cIsUtility>10</style>秒<style=cStack>（每双手环减少1秒。至少1秒）</style>。<color=#FFFF00>工匠特殊效果：充能时间降低至1秒。</color>", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_IGNITEONKILL_DESC", $"击败敌人时产生一次半径<style=cIsDamage>{IgniteOnKillTweak.BaseRadius.ToStdWithStack(IgniteOnKillTweak.StackRadius)}米{"（具体范围与敌人体型相关）".ToUtil()}</style>的爆炸，造成<style=cIsDamage>150%</style>的基础伤害，并<style=cIsDamage>点燃</style>所有被爆炸波及的敌人。<style=cIsDamage>点燃</style>可对敌人造成<style=cIsDamage>75%</style><style=cStack>（每层增加75%）</style>的基础伤害。", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_IMMUNETODEBUFF_DESC", $"增加{BensRainCoat.BaseHealthPercent.Value.ToHealPct() + BensRainCoat.StackHealthPercent.Value.ToStackPct()}最大生命值。免疫<style=cIsUtility>{BensRainCoat.BaseImmunity.Value}个<style=cStack>（每层+{BensRainCoat.StackImmunity.Value}个）</style></style><style=cIsDamage>减益效果</style>，并在触发后的<style=cIsUtility>{BensRainCoat.ImmunityDuration.Value}</style>秒内免疫所有<style=cIsDamage>减益</style>。每<style=cIsUtility>{BensRainCoat.BaseRecharge.Value}</style>秒<style=cStack>（每层减少{BensRainCoat.StackReduction.Value.ToPercent()}）</style>充能一次</style>。", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_INFUSION_DESC", $"每击败一名敌人后使{"自身外加主人".ToHealing() + "（如果有）".ToStack()}<style=cIsHealing>永久性</style>增加<style=cIsHealing>1点</style><style=cStack>（每层+1点）</style>最大生命值，最多增加<style=cIsHealing>自身等级x0.3x自身基础血量x全队层数</style>点。", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_KNURL_DESC", $"{"最大生命值".ToHealing()}增加{TitanicKnurl.BaseHealth.Value.ToHealing() + TitanicKnurl.StackHealth.Value.ToStack("（每层+_）")}点。{"生命值再生".ToHealing()}增加{TitanicKnurl.BaseRegen.Value.ToHealing() + TitanicKnurl.StackRegen.Value.ToStack("（每层+_）")}hp/s，外加{"生命值<color=yellow>百分比</color>再生".ToHealing()}增加{TitanicKnurl.BaseRegenFraction.Value.ToHealPct() + TitanicKnurl.StackRegenFraction.Value.ToStackPct()}hp/s。", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_LIGHTNINGSTRIKEONHIT_DESC", $"命中敌人时有{"10%".ToDmg() + "（概率公式：100%x(层数÷(层数+9))）".ToStack()}的几率向敌人的{"雷电召唤池".ToLightning()}中添加{"1次雷击".ToLightning() + 0.5f.ToStack("（召唤间隔：_秒）")}，造成<style=cIsDamage>300%</style><style=cStack>（每层+300%）</style>的伤害。<color=#FFFF00>装卸工特殊效果：关于“雷霆拳套”（详情看 权杖 技能介绍）。</color>", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_LUNARBADLUCK_DESC", $"所有技能冷却时间缩短<style=cIsUtility>2</style><style=cStack>（每层+1）</style>秒。所有随机效果的概率降低<style=cIsUtility>25%</style>{"（叠加公式：幸运降低概率 = 原概率x(运气/(|运气|+3))）".ToStack()}。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_LUNARSPECIALREPLACEMENT_DESC", $"<style=cIsUtility>将你的特殊技能替换为</style><style=cIsUtility>毁坏</style>。\n\n造成伤害可以施加一层<style=cIsDamage>毁坏</style>，持续10<style=cStack>（每层增加+10秒）秒</style>。启动此技能可以<style=cIsDamage>引爆</style>所有的毁坏层数，不限距离，并造成<style=cIsDamage>300%的伤害</style>，外加<style=cIsDamage>每层毁坏120%<style=cIsLunar>（异教徒：每层+60%）</style>的伤害</style>。8秒<style=cStack>（每层增加8秒）</style>后充能。<style=cIsLunar>异教徒：每层使最大生命值增加50%。</style>", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_LUNARSUN_DESC", $"每<style=cIsUtility>3</style><style=cStack>（每层-50%）</style>秒获得一个<style=cIsDamage>环绕运动的炸弹</style>，碰撞到敌人时爆炸，造成<style=cIsDamage>360%</style>的伤害，最多可获得<style=cIsUtility>3<style=cStack>（每层+1）</style>个炸弹</style>。每<style=cIsUtility>30</style>秒将一件随机物品<style=cIsUtility>{"（排除米斯历克斯不会偷走的物品）".ToStack()}转化</style>为该物品。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_MISSILE_DESC", $"有{"10%".ToDmg() + "（概率公式：100%x(层数÷(层数+9))）".ToStack()}机率向{"导弹发射池".ToUtil()}里添加一枚导弹{ModConfig.导弹发射间隔.Value.ToStack("（发射间隔：_秒）")}。每枚合计造成{"200%".ToDmg() + "（每层增加200%）".ToStack()}的伤害。", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_MISSILEVOID_DESC", $"获得一个相当于你最大生命值<style=cIsHealing>{10}%</style>的<style=cIsHealing>护盾</style>。命中敌人时有{"护盾/总护盾".ToHealing()}x{"100%".ToDmg() + "（忽略触发系数）".ToStack()}机率向敌人发射{"1".ToDmg()}发虾米，合计造成{"护盾/总护盾".ToHealing()}x{MissileVoidTweak.DamageCoefficient.ToPctWithStack().ToDmg()}的伤害。<style=cIsVoid>使所有AtG导弹MK.1无效化</style>。", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_MOVESPEEDONKILL_DESC", $"击杀敌人会使<style=cIsUtility>移动速度</style>提高<style=cIsUtility>125%</style>，在<style=cIsUtility>1</style><style=cStack>（每层+0.5）</style>秒内逐渐失效。<color=#FFFF00>女猎人特殊效果：锁敌距离增加10米{"（每层+10米）".ToStack()}。</color>", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_SIPHONONLOWHEALTH_DESC", $"在战斗状态下，<style=cIsDamage>15米</style>{"（每层+5米）".ToStack()}范围内距离你最近的1<style=cStack>（每层+1）</style>个敌人会与你“拴”在一起，每秒造成<style=cIsDamage>100%</style>{"（每层+100%）".ToStack()}的伤害，施加<style=cIsDamage>焦油</style>效果，造成伤害的<style=cIsHealing>100%</style>转化为对你的<style=cIsHealing>治疗量</style>。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_SPRINTOUTOFCOMBAT_DESC", $"{"移动速度".ToUtil()}提高{RedWhip.BaseMove.Value.ToPercent().ToUtil() + RedWhip.StackMove.Value.ToPercent().ToStack("（每层+_）")}，脱离战斗时会使你的<style=cIsUtility>移动速度</style>提高<style=cIsUtility>{RedWhip.BaseSafeMove.Value.ToPercent()}</style><style=cStack>（每层增加{RedWhip.StackSafeMove.Value.ToPercent()}）</style>。", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_SPRINTWISP_DESC", $"奔跑时自动向周围{LittleDisciple.BaseRange.Value.ToUtil("_米") + LittleDisciple.StackRange.Value.ToStack("（每层+_米）")}内的敌人发射<style=cIsDamage>追踪幽魂</style>，造成{LittleDisciple.BaseDamage.Value.ToDmgPct() + LittleDisciple.StackDamage.Value.ToPercent().ToStack("（每层+_）")}的伤害</style>。奔跑时，每<style=cIsUtility>{LittleDisciple.BaseRecharge.Value}秒</style>{LittleDisciple.StackReduction.Value.ToPercent().ToStack("（每层-_）")}发射一次。{(LittleDisciple.ScaleMove.Value ? "发射速率随<style=cIsUtility>移动速度</style>增加。" : "")}", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_STICKYBOMB_DESC", $"命中敌人时有<style=cIsDamage>5%</style><style=cStack>（每层增加5%）</style>的机率向敌人的{"黏弹喷泉".ToDmg()}中添加{"1颗黏弹".ToDmg() + ModConfig.喷泉喷射间隔.Value.ToStack("（喷射间隔：_秒）")}，爆炸时合计造成<style=cIsDamage>100%</style>伤害。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("KEYWORD_ACTIVERELOAD", $"<style=cKeywordName>主动上弹</style><style=cSub>开火后给你的磁轨炮上弹（按{ModConfig.ReloadKey.Value.MainKey.ToUtil()}键也可进入上弹）。<style=cIsDamage>完美上弹</style>后，下一发射弹额外造成{"50%".ToDmg() + "（每层备用弹夹+10%）".ToStack()}伤害。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("LOADER_SPECIAL_DESCRIPTION", $"扔出飘浮电塔，可<style=cIsDamage>电击</style>周围{M551Pylon.aoe.ToUtil()}米{"（每层不稳定的特斯拉线圈+".ToStack()}内最多<style=cIsDamage>3</style>名敌人，电流最多可弹射{M551Pylon.bounces}次{"（每层不稳定的特斯拉线圈+1次）".ToStack()}，造成{M551Pylon.damage.ToDmgPct()}的伤害。可被<style=cIsUtility>格挡</style>。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("MAGE_SECONDARY_LIGHTNING_DESCRIPTION", $"<style=cIsDamage>眩晕</style>。发射一枚纳米炸弹，如果充能将造成{ChargedNanoBomb.minDamage.ToDmgPct()}-{ChargedNanoBomb.maxDamage.ToDmgPct()}的伤害。飞行期间电击周围<style=cIsUtility>18</style>米{"（每层尤克里里+3米）".ToStack()}内的敌人，每秒造成{"80%".ToDmg()}的伤害。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("MERC_SPECIAL_DESCRIPTION", $"<style=cIsDamage>一闪</style>。瞄准距离最近的敌人，攻击被瞄准的敌人可对其重复造成{Eviscerate.damageCoefficient.ToDmgPct()}的伤害。<style=cIsUtility>过程中无法被攻击，跳跃键可以提前退出技能。</style>。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("RAILGUNNER_ACTIVE_RELOAD_DESCRIPTION", $"在恰到好处的时机上弹可更快恢复，并使你下一发射弹额外造成{"50%".ToDmg() + "（每层备用弹夹+10%）".ToStack()}伤害。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("RAILGUNNER_SNIPE_SUPER_DESCRIPTION", $"发射一枚造成{Supercharge.Damage.ToDmgPct()}的伤害且具有{Supercharge.CritDamage.ToDmg()}倍暴击伤害的超载射弹</style>。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("RAILGUNNER_UTILITY_ALT_DESCRIPTION", $"扔出一部装置，该装置可使附近所有<style=cIsUtility>敌人和射弹</style><style=cIsUtility>减速（射弹减99%）</style>，使所有友方提速{PolarFieldDevice.SpeedBuffVal.ToPercent().ToUtil()}", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("TOOLBOT_SECONDARY_DESCRIPTION", $"<style=cIsDamage>眩晕</style>。发射一枚造成<style=cIsDamage>220%伤害</style>的爆破筒。将分裂为造成<style=cIsDamage>5{"（每层眩晕手雷+5）".ToStack()}x44%伤害</style>以及<style=cIsDamage>眩晕</style>效果的小炸弹。", "zh-CN");
            R2API.LanguageAPI.AddOverlay("TREEBOT_SPECIAL_ALT1_DESCRIPTION", $"发射弩弹，造成{DIRECTIVEHarvest.damage.ToDmgPct()}的伤害且弩弹将<style=cIsDamage>注入</style>一个敌人。此敌人死亡时，掉落多个<style=cIsHealing>果实</style>，可治疗<style=cIsHealing>{DIRECTIVEHarvest.percentHeal.ToPercent()}的生命值</style>{(DIRECTIVEHarvest.giveBuffs ? "，并且给予随机<style=cIsHealing>增益</style>" : "")}。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("VV_ITEM_" + vanillaVoid.Items.ExeBlade.instance.ItemLangTokenName + "_DESCRIPTION", $"你的<style=cIsDamage>击杀效果</style>在杀死一个精英后会额外发生<style=cIsDamage>1</style>次<style=cStack>（每层+1次）</style>。另外会产生半径<style=cIsDamage>12</style>米的<style=cIsDamage>爆炸</style>，造成<style=cIsDamage>100%</style>的伤害。<style=cIsVoid>腐化所有陈旧断头台</style>。</style><color=#FFFF00>圣骑士特殊效果：基础伤害增加3点{"（每层+3点）".ToStack()}。</color>", "zh-CN");
        ...
        public void 权杖技能汉化() {
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_BANDIT2_RESETREVOLVERDESC", Language.GetString("BANDIT2_SPECIAL_DESCRIPTION") + $"额外发射{"1发".ToDmg() + "（每层+1发）".ToStack()}子弹。".ToScepterDescription(), "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_BANDIT2_RESETREVOLVERNAME", "暗杀", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_BANDIT2_SKULLREVOLVERDESC", Language.GetString("BANDIT2_SPECIAL_ALT_DESCRIPTION") + $"子弹有21%{"（每个标记+0.21%）".ToStack()}概率{"（不受运气影响）".ToStack()}弹射到21米内的最多7名敌人{"（每层+7名）".ToStack()}。\n每次弹射后距离和伤害减少{"7%".ToDeath()}。".ToScepterDescription(), "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_BANDIT2_SKULLREVOLVERNAME", "叛徒", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_CAPTAIN_AIRSTRIKEALTDESC", Language.GetString("CAPTAIN_UTILITY_ALT1_DESCRIPTION") + $"自动制导：以{" 自身等级 x 权杖层数 m/s".ToUtil()}的速度追踪敌人。{"莉莉丝".ToRed()}具有{"2倍".ToUtil()}的范围和伤害，并且在打击后产生{"冲击波".ToDmg()}。呼叫{"莉莉丝".ToRed()}将{"消耗6层充能".ToUtil()}，打击抵达前有{"30秒".ToUtil()}等待时间。".ToScepterDescription(), "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_CAPTAIN_AIRSTRIKEALTNAME", "PHN-8300“莉莉丝”打击".ToRed(), "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_CAPTAIN_AIRSTRIKEDESC", Language.GetString("CAPTAIN_UTILITY_DESCRIPTION") + $"按住可连续呼叫UES顺风号，总共可造成{"21x500%".ToDmg() + "（每层+7x500%）".ToStack()}伤害。".ToScepterDescription(), "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_CAPTAIN_AIRSTRIKENAME", "连续轨道炮", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_COMMANDO_BARRAGEDESC", Language.GetString("COMMANDO_SPECIAL_DESCRIPTION") + $"射击速度提高100%{"（每层+100%）".ToStack()}，额外发射{"1发".ToDmg()}子弹。按住你的主要技能可以更精确地射击。".ToScepterDescription(), "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_COMMANDO_BARRAGENAME", "死亡绽放", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_COMMANDO_GRENADEDESC", Language.GetString("COMMANDO_SPECIAL_ALT1_DESCRIPTION") + $"额外扔出{"6".ToDmg() + "（每层+6）".ToStack()}枚具有一半伤害的手雷。".ToScepterDescription(), "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_COMMANDO_GRENADENAME", "地毯式轰炸", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_CROCO_DISEASEDESC", Language.GetString("CROCO_SPECIAL_DESCRIPTION") + $"使受害者成为行走的{"瘟疫之源".ToPoison()}，持续将瘟疫{"传染".ToPoison()}给周围{"10米".ToUtil() + "（每层+10米）".ToStack()}内的敌人。".ToScepterDescription(), "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_CROCO_DISEASENAME", "瘟疫", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_ENGI_TURRETDESC", Language.GetString("ENGI_SPECIAL_DESCRIPTION") + $"可额外放置{"1".ToUtil()}座炮台。".ToScepterDescription(), "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_ENGI_TURRETNAME", "TR12-C 高斯自动炮台", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_ENGI_WALKERDESC", Language.GetString("ENGI_SPECIAL_ALT1_DESCRIPTION") + $"可额外放置{"2".ToUtil()}座炮塔。".ToScepterDescription(), "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_HUNTRESS_BALLISTADESC", Language.GetString("HUNTRESS_SPECIAL_ALT1_DESCRIPTION") + $"每次射击额外发射{"1根".ToDmg() + "（每层+1根）".ToStack()}弩箭。".ToScepterDescription(), "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_HUNTRESS_BALLISTANAME", "-腊包尔->", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_HUNTRESS_RAINDESC", Language.GetString("HUNTRESS_SPECIAL_DESCRIPTION") + $"点燃。自动传送到敌人脚下。半径增加50%，持续时间增加{"0%".ToUtil() + "（每层+100%）".ToStack()}。{"2倍Proc系数".ToUtil()}。".ToScepterDescription(), "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_HUNTRESS_RAINNAME", "火雨", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_LOADER_CHARGEFISTDESC", Language.GetString("LOADER_UTILITY_DESCRIPTION") + $"冲刺速度增加{"100%".ToUtil() + "（每层权杖+100%）".ToStack()}。高到离谱的击退。将{"移速属性".ToUtil()}纳入伤害计算。".ToScepterDescription(), "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_LOADER_CHARGEFISTNAME", "百万吨重拳", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_MAGE_FLAMETHROWERDESC", Language.GetString("MAGE_SPECIAL_FIRE_DESCRIPTION") + $"燃烧将留下{"灼热的火云".ToFire()}，存在{"10秒".ToUtil() + "（每层+10秒）".ToStack()}。".ToScepterDescription(), "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_MAGE_FLAMETHROWERNAME", "龙息", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_MAGE_FLYUPDESC", Language.GetString("MAGE_SPECIAL_LIGHTNING_DESCRIPTION") + $"{"2倍".ToDmg()}伤害、{"5倍".ToUtil()}半径，{"10倍".ToUtil() + "（每层+10倍）".ToStack()}击退。".ToScepterDescription(), "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_MAGE_FLYUPNAME", "反物质浪涌", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_MERC_EVISDESC", Language.GetString("MERC_SPECIAL_DESCRIPTION") + $"持续时间增加{"100%".ToUtil() + "（每层权杖+100%）".ToStack()}。此技能击杀敌人可{"重置持续时间".ToUtil()}，按住跳跃键可在击杀时{"退出".ToDeath()}技能！".ToScepterDescription(), "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_MERC_EVISNAME", "屠戮", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_MERC_EVISPROJDESC", Language.GetString("MERC_SPECIAL_ALT1_DESCRIPTION") + $"刀刃之风飞行速度增加{"100%".ToUtil()}，攻击持续时间增加{"100%".ToUtil() + "（每层+100%）".ToStack()}。".ToScepterDescription(), "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_MERC_EVISPROJNAME", "死亡之风", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_RAILGUNNER_SNIPECRYODESC", Language.GetString("RAILGUNNER_SPECIAL_ALT_DESCRIPTION") + $"每次击中物体将产生{"冰冻".ToIce()}爆炸，{"冰冻".ToIce()}目标敌人{"273.15秒"}并{"冰冻".ToIce()}周围{"27.315米".ToUtil() + "（每层+27.315米）".ToStack()}内的敌人{"27.315秒".ToUtil()}，合计造成{"100%".ToDmg()}伤害。".ToScepterDescription(), "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_RAILGUNNER_SNIPESUPERDESC", Language.GetString("RAILGUNNER_SPECIAL_DESCRIPTION") + $"发射时，将{"25%".ToYellow() + "（消耗公式：100%x(权杖层数÷(权杖层数+3))）".ToStack()}的{"金钱".ToYellow()}转化为伤害。击中将永久降低敌人{"20点".ToDmg()}护甲。Proc系数增加{"0.5".ToUtil()}。".ToScepterDescription(), "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_SCEPLOADER_CHARGEZAPFISTDESC", Language.GetString("LOADER_UTILITY_ALT1_DESCRIPTION") + $"全向{"闪电".ToLightning()}，击中时召唤{"雷电".ToLightning()}，合计造成{"300%".ToDmg() + "（每层电能钻机+300%）".ToStack()}的伤害。冲刺速度增加{"100%".ToUtil() + "（每层权杖+100%）".ToStack()}，冲刺时{"临时提高护甲".ToUtil()}。将{"移速属性".ToUtil()}纳入伤害计算。{"“<link=\"BulwarksHauntShaky\">以雷霆~ 击碎黑暗！</link>”".ToLightning()}".ToScepterDescription(), "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_SCEPLOADER_CHARGEZAPFISTNAME", "雷霆拳套".ToLightning(), "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_TOOLBOT_DASHDESC", Language.GetString("TOOLBOT_UTILITY_DESCRIPTION") + $"进入{"毁灭模式".ToRed()}，可通过{"跳跃按键".ToUtil()}退出。将传入的伤害{"减半".ToUtil()}（与护甲叠加）。退出时产生巨大的{"爆炸".ToDmg()}击晕周围{"20米".ToUtil() + "（每层+20米）".ToStack()}内的敌人，合计造成所受伤害{"200%".ToDmg()}的伤害。".ToScepterDescription(), "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_TOOLBOT_DASHNAME", "毁灭模式".ToRed(), "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_TREEBOT_FLOWER2DESC", Language.GetString("TREEBOT_SPECIAL_DESCRIPTION") + $"花朵的抓取范围增加{"100%".ToUtil() + "（每层+100%）".ToStack()}。造成随机{"减益".ToDeath()}。".ToScepterDescription(), "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_TREEBOT_FLOWER2NAME", "混沌生长", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_TREEBOT_FRUIT2DESC", Language.GetString("TREEBOT_SPECIAL_ALT1_DESCRIPTION") + $"生成额外的{"果实".ToHealing() + "（每层+1基础果实数量）".ToStack()}，{"果实".ToHealing()}给予强大的随机{"增益".ToHealing()}。".ToScepterDescription(), "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_TREEBOT_FRUIT2NAME", "终极命令：收割", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_VOIDSURVIVOR_CRUSHCORRUPTIONDESC", Language.GetString("VOIDSURVIVOR_SPECIAL_DESCRIPTION") + $"技能效果影响周围{"25米".ToUtil() + "（每层+25米）".ToStack()}内的敌人和盟友。".ToScepterDescription(), "zh-CN");
        ...
        public void 圣骑士汉化() {
            ...
            text += "双倍治疗。双倍屏障。清除减益。".ToScepterDescription();
            ...
            text += "更强的减益。更大的范围。摧毁投射物。".ToScepterDescription();
            ...
            text += "更快的施法速度。双倍伤害。双倍攻速。".ToScepterDescription();
            ...
            text += "再次投掷并保持瞄准，然后释放太阳，爆炸对周围<style=cDeath>一切生物</style>造成<style=cIsDamage>4000%伤害</style>。".ToScepterDescription();
            ...
            text += "产生大规模爆炸，造成9000%伤害。".ToScepterDescription();
        ...
        private void MainMenuController_Start(On.RoR2.UI.MainMenu.MainMenuController.orig_Start orig, RoR2.UI.MainMenu.MainMenuController self) {
            orig(self);
            On.RoR2.UI.MainMenu.MainMenuController.Start -= MainMenuController_Start;
            TPDespair.ZetAspects.Language.tokens["zh-CN"]["ITEM_SHIELDONLY_DESC"] += $"\n<style=cIsLunar>完美的象征：护盾百分比再生速度增加{TPDespair.ZetAspects.Configuration.AspectLunarRegen.Value}%{TPDespair.ZetAspects.Configuration.AspectLunarRegen.Value.ToStack("（每层+_%）")}。</style>";
        }
    }
}