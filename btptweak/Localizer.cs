...
namespace BtpTweak {
    ...
    public sealed class Localizer : IEventHandlers {
        ...
        public void 基础汉化() {
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_LUNARSPECIALREPLACEMENT_DESC", $"<style=cIsUtility>将你的特殊技能替换为</style><style=cIsUtility>毁坏</style>。\n\n造成伤害可以施加一层<style=cIsDamage>毁坏</style>{"（异教徒：施加概率提升1000%）".ToLunar()}，持续10<style=cStack>（每层增加+10秒）秒</style>。启动此技能可以<style=cIsDamage>引爆</style>所有的毁坏层数，不限距离，并造成<style=cIsDamage>300%的伤害</style>，外加<style=cIsDamage>每层毁坏120%<style=cIsLunar>（异教徒：每层+60%）</style>的伤害</style>。8秒<style=cStack>（每层增加8秒）</style>后充能。<style=cIsLunar>异教徒：每层使最大生命值增加50%。</style>", "zh-CN");
