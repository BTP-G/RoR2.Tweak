...
namespace BtpTweak {
    ...
    public static class Localization {
        ...
        public static string ToUtil(this object str, string prefix = "", string suffix = "") => "<style=cIsUtility>" + prefix + str + suffix + "</style>";
        ...
        public static void 基础汉化() {
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_SPRINTWISP_DESC", $"奔跑时自动向{LittleDisciple.BaseRange.Value.ToUtil(suffix: "米") + LittleDisciple.StackRange.Value.ToStack("（每层+_米）")}内的敌人发射<style=cIsDamage>追踪幽魂</style>，造成{LittleDisciple.BaseDamage.Value.ToDmgPct() + LittleDisciple.StackDamage.Value.ToPercent().ToStack("（每层+_）")}的伤害</style>。奔跑时，每<style=cIsUtility>{LittleDisciple.BaseRecharge.Value}秒</style>{LittleDisciple.StackReduction.Value.ToPercent().ToStack("（每层-_）")}发射一次。{(LittleDisciple.ScaleMove.Value ? "发射速率随<style=cIsUtility>移动速度</style>增加。" : "")}", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ENGI_SPIDERMINE_DESCRIPTION", $"放置一枚机器人地雷，在敌人走近时自动引爆，造成{ToDmgPct(SpiderMines.damage)}的伤害，最多放置{SpiderMines.charges}枚。", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ENGI_UTILITY_DESCRIPTION", $"放置一个<style=cIsUtility>无法穿透且有击退力的护盾</style>来阻挡弹幕{((BubbleShield.damage > 0f) ? "，并且在击退时造成" + ToDmgPct(BubbleShield.damage / BubbleShield.ticks) + "的伤害" : "")}。每个盾需要消耗{BubbleShield.chargesToConsume}层充能，存在<style=cIsUtility>{BubbleShield.duration}秒</style>。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("EQUIPMENT_LIGHTNING_DESC", $"召唤雷电攻击目标怪物，造成<style=cIsDamage>3000%的伤害</style>并<style=cIsDamage>眩晕</style>附近怪物。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("HUNTRESS_PRIMARY_ALT_DESCRIPTION", $"<style=cIsUtility>灵巧</style>。瞄准60米<style=cStack>（每层猎人的鱼叉+10米）</style>内的敌人，拉弓射出<style=cIsDamage>{Flurry.minArrows}枚</style>跟踪箭，每枚造成{ToDmgPct(Flurry.damage)}的伤害。如果暴击则发射<style=cIsDamage>{Flurry.maxArrows}</style>枚跟踪箭。", "zh-CN");
            R2API.LanguageAPI.AddOverlay("HUNTRESS_PRIMARY_DESCRIPTION", $"<style=cIsUtility>灵巧</style>。瞄准60米<style=cStack>（每层猎人的鱼叉+10米）</style>内的敌人，快速射出一枚能够造成{ToDmgPct(Strafe.damage)}伤害的跟踪箭。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_BLEEDONHITANDEXPLODE_DESC", $"<style=cIsDamage>暴击流血</style>{"（不与其他流血重复）".ToUtil()}将对敌人造成<style=cIsDamage>320%</style>的基础伤害。<style=cIsDamage>流血或出血</style>的敌人将会在死亡时产生半径{"16米".ToDmg() + "（具体范围与敌人体型相关）".ToUtil()}的<style=cIsDamage>爆炸</style>并造成<style=cIsDamage>400%<style=cStack>（每层+400%）</style></style>的基础伤害。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_BOUNCENEARBY_DESC", $"命中时有<style=cIsDamage>33%</style><style=cStack>（每层增加16.5%）</style>的几率向周围{"33米".ToUtil()}内最多<style=cIsDamage>6名</style>敌人发射<style=cIsDamage>追踪钩爪</style>{0.33f.ToStack("（发射间隔：", "s）")}，合计造成<style=cIsDamage>100%的伤害</style>。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_CHAINLIGHTNING_DESC", $"有{"25%".ToDmg() + "（概率公式：100%x(层数÷(层数+3))）".ToStack()}的几率发射<style=cIsDamage>连锁闪电</style>{0.2f.ToStack("（发射间隔：", "s）")}，对<style=cIsDamage>18米</style><style=cStack>（每层增加3米）</style>内的最多<style=cIsDamage>3个</style>{"（每层+1个）".ToStack()}目标合计造成<style=cIsDamage>60%</style>的伤害。<color=#FFFF00>工匠特殊效果：关于“充能完毕的纳米炸弹”（详情看技能介绍）。</color>", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_DEATHMARK_DESC", $"拥有{DeathMark.CountTrigger.Value}个或更多减益效果的敌人<style=cIsDamage>将被标记为死亡</style>，从所有来源受到的伤害增加{DeathMark.BaseDamage.Value.ToDmgPct() + ("（全队每层+" + DeathMark.StackDamage.Value.ToPercent() + "）").ToStack()}，持续{(DeathMark.DebuffDuration.Value + "秒").ToUtil()}。<color=#FFFF00>呛鼻毒师特殊效果：对敌人施加Dot减益的持续时间增加40%{"（每层+40%）".ToStack()}。</color>", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_ELEMENTALRINGVOID_DESC", $"充能完毕后，<style=cIsDamage>不低于400%伤害的攻击</style>击中敌人时会产生一个黑洞，<style=cIsUtility>将15米范围内的敌人吸引至其中心</style>。持续<style=cIsUtility>5</style>秒后坍缩，合计造成<style=cIsDamage>100%</style><style=cStack>（每层+100%）</style>的伤害。坍缩时每多一名敌人在黑洞中就提升{"100%".ToDmg()}的伤害。充能时间<style=cIsUtility>20</style>秒<style=cStack>（每层减少1秒。至少2秒）</style>。<color=#FFFF00>虚空恶鬼特殊效果：充能时间降低至2秒。</color>", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_EXPLODEONDEATH_DESC", $"击败敌人后，召唤一道<style=cIsDamage>岩浆柱</style>对<style=cIsDamage>12米</style>{"（具体范围与敌人体型相关）".ToUtil()}范围内的敌人造成<style=cIsDamage>300%{"（每层+300%）".ToStack()}</style>的基础伤害。", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_EXPLODEONDEATHVOID_DESC", $"命中<style=cIsDamage>未被点燃</style>的敌人时将<style=cIsDamage>引爆</style>它们，产生半径<style=cIsDamage>10米</style>{"（具体范围与敌人体型相关）".ToUtil()}的爆炸，合计造成<style=cIsDamage>50%x{"敌方剩余百分比生命值".ToHealth()}</style>的伤害外加{"100%".ToDmg() + "（每层+100%）".ToStack()}的基础伤害，并且<style=cIsDamage>点燃</style>该敌人，此燃烧伤害{"不享受".ToUtil()}其他加成，燃烧总共造成等同于敌人<style=cIsDamage>剩余生命值</style>的伤害。<style=cIsVoid>使所有鬼火无效化</style>。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_FIREBALLSONHIT_DESC", $"命中时有{"10%".ToDmg() + "（概率公式：100%x(层数÷(层数+9))）".ToStack()}的几率向敌人的{"岩浆球喷泉".ToFire()}中添加{"1颗岩浆球".ToFire() + ModConfig.喷泉喷射间隔.Value.ToStack("（喷射间隔：", "s）")}，造成<style=cIsDamage>300%</style><style=cStack>（每层+300%）</style>的伤害，并<style=cIsDamage>点燃</style>所有敌人。", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_FIRERING_DESC", $"充能完毕后，<style=cIsDamage>不低于400%伤害的攻击</style>击中敌人时会产生一道{"符文火焰龙卷风".ToFire()}轰击敌人，造成<style=cIsDamage>300%</style><style=cStack>（每层+300%）</style>的总持续伤害，同时<style=cIsDamage>点燃</style>敌人。充能时间<style=cIsUtility>10</style>秒<style=cStack>（每双手环减少1秒。至少1秒）</style>。<color=#FFFF00>工匠特殊效果：充能时间降低至1秒。</color>", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_FIREWORK_DESC", $"有{"5%".ToDmg() + "（每层+5%）".ToStack()}机率向{"烟花发射池".ToUtil()}里添加一枚烟花{ModConfig.导弹发射间隔.Value.ToStack("（发射间隔：", "s）")}。每枚合计造成{1f.ToDmgPct()}的伤害。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_ICERING_DESC", $"充能完毕后，<style=cIsDamage>不低于400%伤害的攻击</style>击中敌人时会产生一道{"符文冰霜爆炸".ToIce()}轰击敌人，{"冰冻".ToIce()}周围{"12米".ToUtil()}内的敌人{"2秒".ToUtil()}并使其降低<style=cIsUtility>80%</style>的移动速度<style=cIsUtility>3秒</style><style=cStack>（每层+3秒）</style>，合计造成<style=cIsDamage>300%</style><style=cStack>（每层+300%）</style>的总伤害。充能时间<style=cIsUtility>10</style>秒<style=cStack>（每双手环减少1秒。至少1秒）</style>。<color=#FFFF00>工匠特殊效果：充能时间降低至1秒。</color>", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_IGNITEONKILL_DESC", $"击败敌人时产生一次半径<style=cIsDamage>12米{"（每层+4米）".ToStack() + "（具体范围与敌人体型相关）".ToUtil()}</style>的爆炸，造成<style=cIsDamage>150%</style>的基础伤害，并<style=cIsDamage>点燃</style>所有被爆炸波及的敌人。<style=cIsDamage>点燃</style>可对敌人造成<style=cIsDamage>75%</style><style=cStack>（每层增加75%）</style>的基础伤害。", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_IMMUNETODEBUFF_DESC", $"增加{BensRainCoat.BaseHealthPercent.Value.ToHealPct() + ("（每层+" + 100 * BensRainCoat.StackHealthPercent.Value + "%）").ToStack()}最大生命值。免疫<style=cIsUtility>{BensRainCoat.BaseImmunity.Value}个<style=cStack>（每层+{BensRainCoat.StackImmunity.Value}个）</style></style><style=cIsDamage>减益效果</style>，并在触发后的<style=cIsUtility>{BensRainCoat.ImmunityDuration.Value}</style>秒内免疫所有<style=cIsDamage>减益</style>。每<style=cIsUtility>{BensRainCoat.BaseRecharge.Value}</style>秒<style=cStack>（每层减少{100 * BensRainCoat.StackReduction.Value}%）</style>充能一次</style>。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_LIGHTNINGSTRIKEONHIT_DESC", $"命中时有{"10%".ToDmg() + "（概率公式：100%x(层数÷(层数+9))）".ToStack()}的几率向敌人的{"雷电召唤池".ToLightning()}中添加{"1次雷击".ToLightning() + 0.5f.ToStack("（基础召唤间隔：", "s）")}，造成<style=cIsDamage>300%</style><style=cStack>（每层+300%）</style>的伤害。<color=#FFFF00>装卸工特殊效果：关于“雷霆拳套”（详情看 权杖 技能介绍）。</color>", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_MISSILE_DESC", $"有{"10%".ToDmg() + "（概率公式：100%x(层数÷(层数+9))）".ToStack()}机率向{"导弹发射池".ToUtil()}里添加一枚导弹{ModConfig.导弹发射间隔.Value.ToStack("（发射间隔：", "s）")}。每枚合计造成{"200%".ToDmg() + "（每层增加200%）".ToStack()}的伤害。", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ITEM_MISSILEVOID_DESC", $"获得一个相当于你最大生命值<style=cIsHealing>{10}%</style>的<style=cIsHealing>护盾</style>。命中时有{"百分比护盾值x".ToHealing() + "33%".ToDmg() + "（每层+33%）".ToStack()}机率向敌人发射{"3".ToDmg()}发虾米，每发合计造成{"百分比护盾值x".ToHealing()}<style=cIsDamage>{60}%</style><style=cStack>（每层+{60}%）</style>的伤害。<style=cIsVoid>使所有AtG导弹MK.1无效化</style>。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ITEM_STICKYBOMB_DESC", $"命中时有<style=cIsDamage>5%</style><style=cStack>（每层增加5%）</style>的机率向敌人的{"黏弹喷泉".ToDmg()}中添加{"1颗黏弹".ToDmg() + ModConfig.喷泉喷射间隔.Value.ToStack("（喷射间隔：", "s）")}，爆炸时合计造成<style=cIsDamage>100%</style>伤害。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("MAGE_SECONDARY_ICE_DESCRIPTION", $"<style=cIsUtility>冰冻</style>。使用拥有<style=cIsDamage>穿透</style>效果的纳米枪发动攻击，充能后能造成{ToDmgPct(CastNanoSpear.minDamage)}-{ToDmgPct(CastNanoSpear.maxDamage)}的伤害。爆炸后留下一个<style=cIsUtility>冰冻炸弹</style>，2秒后爆炸并<style=cIsUtility>冰冻</style>附近{"12米".ToUtil()}的敌人，合计造成{"100%".ToDmg()}的伤害。", "zh-CN");
            R2API.LanguageAPI.AddOverlay("MAGE_SECONDARY_LIGHTNING_DESCRIPTION", $"<style=cIsDamage>眩晕</style>。发射一枚纳米炸弹，如果充能将造成{ToDmgPct(ChargedNanoBomb.minDamage)}-{ToDmgPct(ChargedNanoBomb.maxDamage)}的伤害。飞行期间电击周围<style=cIsUtility>18</style>米{"（每层尤克里里+3米）".ToStack()}内的敌人，每秒造成{"80%".ToDmg()}的伤害。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("RAILGUNNER_SECONDARY_ALT_DESCRIPTION", $"{"手动上弹".ToUtil()}。启动你的<style=cIsUtility>近程瞄准镜</style>，高亮显示<style=cIsDamage>弱点</style>，并将你的武器转化为一门可造成{ToDmgPct(HH44Marksman.Damage)}伤害的快速磁轨炮。", "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("RAILGUNNER_SNIPE_SUPER_DESCRIPTION", $"发射一枚造成{ToDmgPct(Supercharge.Damage)}的伤害且具有{Supercharge.CritDamage.ToDmg()}倍暴击伤害的超载射弹</style>。", "zh-CN");
        ...
        public static void 权杖技能汉化() {
            ...
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_BANDIT2_SKULLREVOLVERDESC", Language.GetString("BANDIT2_SPECIAL_ALT_DESCRIPTION") + ToScepterDescription($"子弹有21%{ToStack("（每个标记+0.21%）")}概率{ToStack("（不受运气影响）")}弹射到21米内的最多7名敌人{"（每层+7名）".ToStack()}。\n每次弹射后距离和伤害减少{"7%".ToDeath()}。"), "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_CAPTAIN_AIRSTRIKEDESC", Language.GetString("CAPTAIN_UTILITY_DESCRIPTION") + ToScepterDescription($"按住可连续呼叫UES顺风号，总共可造成{"21x500%".ToDmg() + "（每层+7x500%）".ToStack()}伤害。"), "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_CAPTAIN_AIRSTRIKENAME", "连续轨道炮", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_COMMANDO_BARRAGEDESC", Language.GetString("COMMANDO_SPECIAL_DESCRIPTION") + ToScepterDescription($"射击速度提高100%{"（每层+100%）".ToStack()}，额外发射{"1发".ToDmg()}子弹。按住你的主要技能可以更精确地射击。"), "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_COMMANDO_BARRAGENAME", "死亡绽放", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_COMMANDO_GRENADEDESC", Language.GetString("COMMANDO_SPECIAL_ALT1_DESCRIPTION") + ToScepterDescription($"额外扔出{"6".ToDmg() + "（每层+6）".ToStack()}枚具有一半伤害的手雷。"), "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_COMMANDO_GRENADENAME", "地毯式轰炸", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_CROCO_DISEASEDESC", Language.GetString("CROCO_SPECIAL_DESCRIPTION") + ToScepterDescription($"使受害者成为行走的{"瘟疫之源".ToPoison()}，持续将瘟疫{"传染".ToPoison()}给周围{"10米".ToUtil() + "（每层+10米）".ToStack()}内的敌人。"), "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_HUNTRESS_BALLISTADESC", Language.GetString("HUNTRESS_SPECIAL_ALT1_DESCRIPTION") + ToScepterDescription($"每次射击额外发射{"1根".ToDmg() + "（每层+1根）".ToStack()}弩箭。"), "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_HUNTRESS_BALLISTANAME", "-腊包尔->", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_HUNTRESS_RAINDESC", Language.GetString("HUNTRESS_SPECIAL_DESCRIPTION") + ToScepterDescription($"点燃。自动传送到敌人脚下。半径增加50%，持续时间增加{"0%".ToUtil() + "（每层+100%）".ToStack()}。{"2倍Proc系数".ToUtil()}。"), "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_MAGE_FLAMETHROWERDESC", Language.GetString("MAGE_SPECIAL_FIRE_DESCRIPTION") + ToScepterDescription($"燃烧将留下{"灼热的火云".ToFire()}，存在{"10秒".ToUtil() + "（每层+10秒）".ToStack()}。"), "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_MAGE_FLAMETHROWERNAME", "龙息", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_MAGE_FLYUPDESC", Language.GetString("MAGE_SPECIAL_LIGHTNING_DESCRIPTION") + ToScepterDescription($"{"2倍".ToDmg()}伤害、{"5倍".ToUtil()}半径，{"10倍".ToUtil() + "（每层+10倍）".ToStack()}击退。"), "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_MERC_EVISPROJDESC", Language.GetString("MERC_SPECIAL_ALT1_DESCRIPTION") + ToScepterDescription($"刀刃之风飞行速度增加{"100%".ToUtil()}，攻击持续时间增加{"100%".ToUtil() + "（每层+100%）".ToStack()}。"), "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_MERC_EVISPROJNAME", "死亡之风", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_RAILGUNNER_SNIPECRYODESC", Language.GetString("RAILGUNNER_SPECIAL_ALT_DESCRIPTION") + ToScepterDescription($"每次击中物体将产生{"冰冻".ToIce()}爆炸，{"冰冻".ToIce()}目标敌人{"273.15秒"}并{"冰冻".ToIce()}周围{"27.315米".ToUtil() + "（每层+27.315米）".ToStack()}内的敌人{"27.315秒".ToUtil()}，合计造成{"100%".ToDmg()}伤害。"), "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_RAILGUNNER_SNIPESUPERDESC", Language.GetString("RAILGUNNER_SPECIAL_DESCRIPTION") + ToScepterDescription($"发射时，将{"25%".ToYellow() + "（消耗公式：100%x(权杖层数÷(权杖层数+3))）".ToStack()}的{"金钱".ToYellow()}转化为伤害。击中将永久降低敌人{"20点".ToDmg()}护甲。Proc系数增加{"0.5".ToUtil()}。"), "zh-CN");
            ...
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_TOOLBOT_DASHDESC", Language.GetString("TOOLBOT_UTILITY_DESCRIPTION") + ToScepterDescription($"进入{"毁灭模式".ToRed()}，可通过{"跳跃按键".ToUtil()}退出。将传入的伤害{"减半".ToUtil()}（与护甲叠加）。退出时产生巨大的{"爆炸".ToDmg()}击晕周围{"20米".ToUtil() + "（每层+20米）".ToStack()}内的敌人，合计造成所受伤害{"200%".ToDmg()}的伤害。"), "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_TOOLBOT_DASHNAME", "毁灭模式".ToRed(), "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_TREEBOT_FLOWER2DESC", Language.GetString("TREEBOT_SPECIAL_DESCRIPTION") + ToScepterDescription($"花朵的抓取范围增加{"100%".ToUtil() + "（每层+100%）".ToStack()}。造成随机{"减益".ToDeath()}。"), "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_TREEBOT_FLOWER2NAME", "混沌生长", "zh-CN");
            R2API.LanguageAPI.AddOverlay("ANCIENTSCEPTER_TREEBOT_FRUIT2DESC", Language.GetString("TREEBOT_SPECIAL_ALT1_DESCRIPTION") + ToScepterDescription($"生成额外的{"果实".ToHealing() + "（每层+1基础果实数量）".ToStack()}，{"果实".ToHealing()}给予强大的随机{"增益".ToHealing()}。"), "zh-CN");
