...
namespace BtpTweak {
    ...
    public static class Localization {
        ...
        public static string ToDmg(this float damage, string start = "", string end = "%") => "<style=cIsDamage>" + start + 100 * damage + end + "</style>";
        ...
        public static void 基础汉化() {
            ...
            AddOverlay("EQUIPMENT_BFG_DESC", $"发射前子卷须，对{"66.6".ToUtil()}米范围内的敌人造成最高<style=cIsDamage>每秒666%的伤害</style>。接触目标并引爆后，会对20米范围内的敌人造成<style=cIsDamage>6666%的伤害</style>。", "zh-CN");
            AddOverlay("EQUIPMENT_LIGHTNING_DESC", $"召唤雷电攻击目标怪物，造成<style=cIsDamage>3000%{"（每层电能钻机+3000%）".ToStack()}的伤害</style>并<style=cIsDamage>眩晕</style>附近怪物。", "zh-CN");
            AddOverlay("EQUIPMENT_FIREBALLDASH_DESC", $"变成<style=cIsDamage>龙之火球</style>持续<style=cIsDamage>6</style>秒，受击可造成<style=cIsDamage>500%的伤害</style>，飞行时持续喷射造成{"400%".ToDmg() + "（每层熔融钻机+400%）".ToDmg()}伤害的岩浆球。结束时会引爆，造成<style=cIsDamage>800%的伤害</style>。", "zh-CN");
            AddOverlay("VOIDCRID_FLAMEBREATH", "火焰吐息", "zh-CN");
            AddOverlay("EQUIPMENT_GOLDGAT_DESC", $"发射一阵钱雨，<style=cIsDamage>每颗子弹均造成100%{"（每层成卷的零钱+100%）".ToStack()}，外加等同于消耗金钱的伤害</style>。每颗子弹消耗1枚{"（启动后，每秒+1枚）".ToStack()}金钱，价格随游戏时间不断上升。", "zh-CN");
            ...
            AddOverlay("ITEM_AUTOCASTEQUIPMENT_DESC", $"<style=cIsUtility>使装备冷却时间减少</style><style=cIsUtility>50%</style><style=cStack>（每层增加15%）</style>，但会使装备增加{"0.15秒强制冷却时间".ToUtil() + "（每层+0.15秒）".ToStack()}。装备会在<style=cIsUtility>冷却时间</style>结束时被迫自动<style=cIsUtility>激活</style>。", "zh-CN");
            AddOverlay("ITEM_FIREBALLSONHIT_DESC", $"命中时有<style=cIsDamage>20%</style>的几率向敌人的{"岩浆球喷泉".ToUtil()}中添加{"1颗岩浆球".ToDmg()}，造成<style=cIsDamage>400%</style><style=cStack>（每层+400%）</style>的伤害，并<style=cIsDamage>点燃</style>所有敌人。在{0.5f.ToUtil()}秒内添加具有相同触发链的{"岩浆球".ToDmg()}将会整合为{1f.ToUtil()}颗，且在整合时{"无伤害损失".ToUtil()}。", "zh-CN");
            AddOverlay("ITEM_LIGHTNINGSTRIKEONHIT_DESC", $"命中时有<style=cIsDamage>20%</style>的几率向敌人的{"雷电球喷泉".ToUtil()}中添加{"1颗雷电球".ToDmg()}，造成<style=cIsDamage>400%</style><style=cStack>（每层+400%）</style>的伤害。在{0.5f.ToUtil()}秒内添加具有相同触发链的{"雷电球".ToDmg()}将会整合为{1f.ToUtil()}颗，且在整合时{"无伤害损失".ToUtil()}。<color=#FFFF00>装卸工特殊效果：关于“雷霆拳套”（详情看 权杖 技能介绍）。</color>", "zh-CN");
            ...
            AddOverlay("CAPTAIN_SUPPLY_EQUIPMENT_RESTOCK_DESCRIPTION", $"使用时{ToUtil("为装备充能")}。消耗的能量随时间自动填充，速度随等级提升。", "zh-CN");
            AddOverlay("CAPTAIN_SUPPLY_HEAL_DESCRIPTION", $"每秒为附近10米{ToStack("（每级+1米）")}所有友方<style=cIsHealing>恢复</style>等同于各个角色<style=cIsHealing>最大生命值</style>{ToHeal("10%", end: ToStack("（每级+1%）"))}的生命值。", "zh-CN");
            AddOverlay("CAPTAIN_SUPPLY_SHOCKING_DESCRIPTION", $"间歇性<style=cIsDamage>震荡</style>附近20米的所有敌人，使其无法移动。", "zh-CN");
            AddOverlay("CAPTAIN_UTILITY_ALT1_DESCRIPTION", $"<style=cIsDamage>眩晕</style>。{ToUtil("消耗4层充能")}向<style=cIsDamage>UES顺风号</style>请求发动一次<style=cIsDamage>动能打击</style>。在<style=cIsUtility>20秒</style>后，对所有角色造成{"50000%".ToDmg()}的伤害。", "zh-CN");
            ...
            AddOverlay("COMMANDO_SECONDARY_DESCRIPTION", $"发射一枚<style=cIsDamage>穿甲</style>弹，造成{ToDmg(PhaseRound.Damage)}的伤害。每次穿透敌人，造成的伤害提高<style=cIsDamage>40%</style>。", "zh-CN");
            AddOverlay("COMMANDO_SPECIAL_ALT1_DESCRIPTION", $"扔出一枚手雷，爆炸可造成{ToDmg(FragGrenade.Damage)}的伤害。最多可投掷2枚。", "zh-CN");
            AddOverlay("COMMANDO_SPECIAL_DESCRIPTION", $"<style=cIsDamage>眩晕</style>。连续射击，每枚弹丸造成{ToDmg(SuppressiveFire.Damage)}的伤害。射击次数随攻击速度增加。", "zh-CN");
            AddOverlay("ENGI_PRIMARY_DESCRIPTION", $"发射<style=cIsDamage>{BouncingGrenades.maximumGrenadesCount}</style>颗手雷，每颗均可造成{ToDmg(BouncingGrenades.damage)}的伤害。", "zh-CN");
            ...
            AddOverlay("ENGI_SKILL_HARPOON_DESCRIPTION", $"进入<style=cIsUtility>目标标记模式</style>以发射热追踪鱼叉导弹，每发造成{ToDmg(ThermalHarpoons.damage)}的伤害。最多可储存{ThermalHarpoons.charges}发。", "zh-CN");
            AddOverlay("ENGI_SPECIAL_ALT1_DESCRIPTION", $"放置一个<style=cIsUtility>移动</style>炮塔可<style=cIsUtility>继承你所有物品</style>。发射的激光可造成<style=cIsDamage>每秒200%的伤害</style>，并可<style=cIsUtility>减速敌人</style>，最多放置2座。{ToStack("（炮塔会继承浸剂加成，塔击杀敌人，所有队员都可以收到浸剂奖励）")}", "zh-CN");
            AddOverlay("ENGI_SPECIAL_DESCRIPTION", $"放置一个<style=cIsUtility>继承你所有物品</style>的炮塔，发射的炮弹可造成<style=cIsDamage>100%的伤害</style>，最多放置2座。{ToStack("（炮塔会继承浸剂加成，炮塔击杀敌人，所有队员都可以收到浸剂奖励）")}", "zh-CN");
            AddOverlay("ENGI_SPIDERMINE_DESCRIPTION", $"放置一枚机器人地雷，在敌人走近时自动引爆，造成{ToDmg(SpiderMines.damage)}的伤害，最多放置{SpiderMines.charges}枚<style=cStack>（每级+1枚）</style>。", "zh-CN");
            AddOverlay("ENGI_UTILITY_DESCRIPTION", $"放置一个<style=cIsUtility>无法穿透且有击退力的护盾</style>来阻挡弹幕{((BubbleShield.damage > 0f) ? "，并且在击退时造成" + ToDmg(BubbleShield.damage / BubbleShield.ticks) + "的伤害" : "")}。每个盾需要消耗{BubbleShield.chargesToConsume}层充能，存在<style=cIsUtility>{BubbleShield.duration}秒</style>。<style=cIsUtility>护盾</style>展开时为每个友方单位提供一个持续<style=cIsUtility>6秒</style>的<style=cIsUtility>个人护盾</style>。", "zh-CN");
            AddOverlay("GOLDENKNURL_DESC", $"{ToHeal(100 * GoldenCoastPlus.GoldenCoastPlus.KnurlHealth.Value, "最大生命值增加", "%" + ToStack("（每层+" + 100 * GoldenCoastPlus.GoldenCoastPlus.KnurlHealth.Value + "%）"))}，{ToHeal("生命值再生")}增加{ToHeal(GoldenCoastPlus.GoldenCoastPlus.KnurlRegen.Value + "hp/s" + ToStack("（每层+" + GoldenCoastPlus.GoldenCoastPlus.KnurlRegen.Value + "hp/s）"))}，{ToUtil("护甲")}增加{ToUtil(GoldenCoastPlus.GoldenCoastPlus.KnurlArmor.Value + "点", end: ToStack("（每层 + " + GoldenCoastPlus.GoldenCoastPlus.KnurlArmor.Value + "点）"))}。 \n<color=yellow>黄金</color>：{ToHeal("再生速度")}提升{ToHeal("50 % ", end: ToStack("（每层 + 50 %；每有1, 000, 000金钱 + 1 %）"))}。", "zh-CN");
            AddOverlay("GOLDENKNURL_NAME", "<color=yellow>黄金隆起</color>", "zh-CN");
            AddOverlay("GOLDENKNURL_PICKUP", "增加最大生命值、生命值再生速度和护甲。", "zh-CN");
            AddOverlay("HAT_MAGE_UTILITY_FIRE_DESCRIPTION", $"<style=cIsUtility>灵巧</style>。<style=cIsDamage>点燃</style>。向前冲刺，在身后召唤造成{ToDmg(HIFUArtificerTweaks.Main.flamewallDamage.Value, "每秒")}的伤害的火柱</style>。", "zh-CN");
            AddOverlay("HAT_MAGE_UTILITY_FIRE_NAME", "火墙", "zh-CN");
            AddOverlay("HUNTRESS_PRIMARY_ALT_DESCRIPTION", $"<style=cIsUtility>灵巧</style>。瞄准60米<style=cStack>（每升一级+{ModConfig.女猎人射程每级增加距离_.Value}米）</style>内的敌人，拉弓射出<style=cIsDamage>{Flurry.minArrows}枚</style>跟踪箭，每枚造成{ToDmg(Flurry.damage)}的伤害。如果暴击则发射<style=cIsDamage>{Flurry.maxArrows}</style>枚跟踪箭。", "zh-CN");
            AddOverlay("HUNTRESS_PRIMARY_DESCRIPTION", $"<style=cIsUtility>灵巧</style>。瞄准60米<style=cStack>（每升一级+{ModConfig.女猎人射程每级增加距离_.Value}米）</style>内的敌人，快速射出一枚能够造成{ToDmg(Strafe.damage)}伤害的跟踪箭。", "zh-CN");
            AddOverlay("HUNTRESS_SECONDARY_DESCRIPTION", $"{(LaserGlaive.agile ? "<style=cIsUtility>灵巧</style>。" : "")}投掷一把追踪月刃，可弹射最多<style=cIsDamage>{LaserGlaive.bounceCount}</style>次，初始造成{ToDmg(LaserGlaive.damage)}的伤害，每次弹射伤害增加<style=cIsDamage>{Math.Round((double)((LaserGlaive.bounceDamage - 1f) * 100f), 1)}%</style>。", "zh-CN");
            AddOverlay("HUNTRESS_SPECIAL_ALT1_DESCRIPTION", $"向后<style=cIsUtility>传送</style>至空中。最多发射<style=cIsDamage>{Ballista.boltCount}</style>道能量闪电，造成<style=cIsDamage>{Ballista.boltCount}x{ToDmg(Ballista.damage)}的伤害</style>。", "zh-CN");
            AddOverlay("HUNTRESS_SPECIAL_DESCRIPTION", $"<style=cIsUtility>传送</style>至空中，向目标区域射下箭雨，使区域内所有敌人<style=cIsUtility>减速</style>，并造成{ToDmg(3f * ArrowRain.damage, "每秒")}的伤害。", "zh-CN");
            ...
            AddOverlay("ITEM_FIREWORK_DESC", $"有{"5%".ToDmg() + "（每层+5%）".ToStack()}机率向{"烟花发射池".ToUtil()}里添加一枚烟花。每枚合计造成{1.5f.ToDmg()}的伤害。在{0.5f.ToUtil()}秒内添加具有相同触发链的烟花将会整合为{1f.ToUtil()}枚，且在整合时{"有伤害损失".ToUtil()}。", "zh-CN");
            ...
            AddOverlay("ITEM_KNURL_DESC", $"{ToHeal("最大生命值")}增加{ToHeal("100点", end: ToStack("（每层+100点）"))}。{ToHeal("生命值再生增加4.8hp/s", end: ToStack("（每层+4.8hp/s）"))}，外加{ToHeal("生命值<color=yellow>百分比</color>再生")}增加{ToHeal("2.4%hp/s", end: ToStack("（每层+2.4%hp/s）"))}。", "zh-CN");
            ...
            AddOverlay("ITEM_LUNARWINGS_DESC", $"{"古代鸟羽：仅存于完整之时。".ToUtil()}\n获得对所有物品的控制权。", "zh-CN");
            AddOverlay("ITEM_LUNARWINGS_NAME", "<style=cIsLunar>特拉法梅的祝福</style>", "zh-CN");
            AddOverlay("ITEM_LUNARWINGS_PICKUP", "一双翅膀。", "zh-CN");
            AddOverlay("ITEM_MISSILE_DESC", $"有{"25%".ToDmg()}机率向{"导弹发射池".ToUtil()}里添加一枚导弹。每枚合计造成{2.5f.ToDmg() + "（每层增加250%）".ToStack()}的伤害。在{0.5f.ToUtil()}秒内添加具有相同触发链的导弹将会整合为{1f.ToUtil()}枚，且在整合时{"有伤害损失".ToUtil()}。", "zh-CN");
            AddOverlay("ITEM_RANDOMLYLUNAR_DESC", $"月球切片的使用次数增加{ToUtil("3次", end: ToStack("（每层+3次）"))}。", "zh-CN");
            ...
            AddOverlay("KEYWORD_SOULROT", $"<style=cKeywordName>灵魂之痛</style><style=cSub>每秒<style=cIsVoid>至少</style>造成敌人<style=cIsHealing>最大生命值</style>的<style=cIsVoid>2.5%</style>的伤害，持续{ToDmg(20, "秒", ToStack("（每层权杖+10秒）"))}后消失。</style>", "zh-CN");
            AddOverlay("KEYWORD_VERY_HEAVY", "<style=cKeywordName>超重</style><style=cSub>下落速度越快，技能造成的伤害越高。", "zh-CN");
            AddOverlay("LOADER_SPECIAL_ALT_DESCRIPTION", $"<style=cIsUtility>超重</style>。用重拳砸向地面，造成{ToDmg(Thunderslam.damage)}的伤害。", "zh-CN");
            AddOverlay("LOADER_SPECIAL_DESCRIPTION", $"扔出飘浮电塔，可<style=cIsDamage>电击</style>周围{M551Pylon.aoe.ToUtil()}米{ToStack("（每层不稳定的特斯拉线圈+" + M551Pylon.aoe + "米）")}内最多<style=cIsDamage>3</style>名敌人，电流最多可弹射{M551Pylon.bounces}次{ToStack("（每层不稳定的特斯拉线圈+1次）")}，造成{ToDmg(M551Pylon.damage)}的伤害。可被<style=cIsUtility>格挡</style>。", "zh-CN");
            ...
            AddOverlay("MAGE_SECONDARY_ICE_DESCRIPTION", $"<style=cIsUtility>冰冻</style>。使用拥有<style=cIsDamage>穿透</style>效果的纳米枪发动攻击，充能后能造成{ToDmg(CastNanoSpear.minDamage)}-{ToDmg(CastNanoSpear.maxDamage)}的伤害。爆炸后留下一个等同于纳米枪伤害的<style=cIsUtility>冰冻炸弹</style>，2秒后爆炸，<style=cIsUtility>冰冻</style>附件10米{ToStack("（每层鲁纳德的手环+1米，如果暴击则使范围翻倍）")}的敌人。", "zh-CN");
            AddOverlay("MAGE_SECONDARY_LIGHTNING_DESCRIPTION", $"<style=cIsDamage>眩晕</style>。发射一枚纳米炸弹，如果充能将造成{ToDmg(ChargedNanoBomb.minDamage)}-{ToDmg(ChargedNanoBomb.maxDamage)}的伤害{ToStack("（每层尤克里里使伤害增加10%）")}。爆炸后电击周围<style=cIsUtility>30</style>米{ToStack("（每层尤克里里+3米）")}内的<style=cIsUtility>3</style>名{ToStack("（每层尤克里里+1名）")}敌人。", "zh-CN");
            AddOverlay("MAGE_SPECIAL_FIRE_DESCRIPTION", $"<style=cIsDamage>点燃</style>。灼烧面前的所有敌人，对其造成{ToDmg(Flamethrower.Damage + Flamethrower.Damage / (1f / (Flamethrower.BurnChance / 100f)), "每秒")}的伤害。", "zh-CN");
            AddOverlay("MAGE_UTILITY_ICE_DESCRIPTION", $"<style=cIsUtility>冰冻</style>。创造一道能够对敌人造成{ToDmg(Snapfreeze.damage)}伤害的屏障。", "zh-CN");
            ...
            AddOverlay("MERC_SECONDARY_ALT1_DESCRIPTION", $"<style=cIsDamage>一闪</style>。释放一个裂片上勾拳，造成{ToDmg(6)}的伤害，并将你送到半空。", "zh-CN");
            ...
            AddOverlay("MERC_SPECIAL_ALT1_DESCRIPTION", $"<style=cIsDamage>一闪</style>。发射一次刀刃之风，最多可对<style=cIsDamage>3</style>名敌人造成<style=cIsDamage>8x{ToDmg(SlicingWinds.damage)}的伤害</style>。最后一次打击将<style=cIsUtility>暴露</style>敌人。", "zh-CN");
            AddOverlay("MERC_SPECIAL_DESCRIPTION", $"<style=cIsDamage>一闪</style>。瞄准距离最近的敌人，攻击被瞄准的敌人可对其重复造成{ToDmg(Eviscerate.damageCoefficient)}的伤害。<style=cIsUtility>过程中无法被攻击，跳跃键可以提前退出技能。</style>。", "zh-CN");
            ...
            AddOverlay("RAILGUNNER_ACTIVE_RELOAD_DESCRIPTION", $"在恰到好处的时机上弹可更快恢复，并使你下一发射弹额外造成{ScopeAndReload.Damage.ToDmg() + "（每层备用弹夹+400%）".ToStack()}的基础伤害。", "zh-CN");
            AddOverlay("RAILGUNNER_PRIMARY_DESCRIPTION", $"发射主动追踪弹药，造成<style=cIsDamage>100%的伤害</style>。", "zh-CN");
            AddOverlay("RAILGUNNER_SECONDARY_ALT_DESCRIPTION", $"启动你的<style=cIsUtility>近程瞄准镜</style>，高亮显示<style=cIsDamage>弱点</style>，并将你的武器转化为一门可造成{ToDmg(HH44Marksman.Damage)}伤害的快速磁轨炮。", "zh-CN");
            AddOverlay("RAILGUNNER_SECONDARY_DESCRIPTION", $"启动你的<style=cIsUtility>远程瞄准镜</style>，高亮显示<style=cIsDamage>弱点</style>，并将你的武器转化为一门可造成{ToDmg(M99Sniper.Damage)}伤害的穿刺磁轨炮。", "zh-CN");
            AddOverlay("RAILGUNNER_SNIPE_CRYO_DESCRIPTION", $"<style=cIsUtility>冰冻</style>。发射一枚超低温射弹，造成{ToDmg(Cryocharge.Damage)}的伤害。", "zh-CN");
            AddOverlay("RAILGUNNER_SNIPE_HEAVY_DESCRIPTION", $"发射一枚重型射弹，造成{ToDmg(M99Sniper.Damage)}的伤害。", "zh-CN");
            AddOverlay("RAILGUNNER_SNIPE_LIGHT_DESCRIPTION", $"发射一枚轻型射弹，造成{ToDmg(HH44Marksman.Damage)}的伤害。", "zh-CN");
            AddOverlay("RAILGUNNER_SNIPE_SUPER_DESCRIPTION", $"发射一枚造成{ToDmg(Supercharge.Damage)}的伤害且具有{Supercharge.CritDamage}倍暴击伤害的超载射弹</style>。", "zh-CN");
            AddOverlay("RAILGUNNER_SPECIAL_ALT_DESCRIPTION", $"<style=cIsUtility>冰冻</style>。发射<style=cIsDamage>具有穿透效果</style>的子弹，造成{ToDmg(Cryocharge.Damage)}的伤害。", "zh-CN");
            AddOverlay("RAILGUNNER_SPECIAL_DESCRIPTION", $"发射一枚<style=cIsDamage>具有穿刺效果，</style>造成{ToDmg(Supercharge.Damage)}%的伤害且具有{Supercharge.CritDamage}倍暴击伤害</style>的超载射弹。之后，<style=cIsHealth>你的所有武器都将失灵</style>，持续<style=cIsHealth>{Supercharge.HopooBalance}</style>秒。", "zh-CN");
            AddOverlay("RAILGUNNER_UTILITY_ALT_DESCRIPTION", $"扔出一部装置，该装置可使附近所有<style=cIsUtility>敌人和射弹</style><style=cIsUtility>减速（射弹减99%）</style>，使所有友方提速{ToUtil((100 * PolarFieldDevice.SpeedBuffVal).ToString() + "%")}", "zh-CN");
            ...
            AddOverlay("TREEBOT_SPECIAL_ALT1_DESCRIPTION", $"发射弩弹，造成{ToDmg(DIRECTIVEHarvest.damage)}的伤害且弩弹将<style=cIsDamage>注入</style>一个敌人。此敌人死亡时，掉落多个<style=cIsHealing>果实</style>，可治疗<style=cIsHealing>{100 * DIRECTIVEHarvest.percentHeal}%的生命值</style>{(DIRECTIVEHarvest.giveBuffs ? "，并且给予随机<style=cIsHealing>增益</style>" : "")}。", "zh-CN");
            AddOverlay("TREEBOT_SPECIAL_ALT1_NAME", "命令：收获", "zh-CN");
            AddOverlay("TREEBOT_SPECIAL_DESCRIPTION", $"<style=cIsHealth>25%生命值</style>。发射一朵会<style=cIsDamage>扎根</style>并造成<style=cIsDamage>200%伤害</style>的花朵。{ToHeal("每命中一个目标便会对你进行一次治疗", end: ToStack("（每层轻粒子雏菊增加治疗量）"))}。", "zh-CN");
            AddOverlay("TREEBOT_UTILITY_ALT1_DESCRIPTION", $"<style=cIsHealth>{100 * BrambleVolley.healthCost}%生命值</style>。发射一次<style=cIsUtility>音爆</style>并对敌人造成{ToDmg(BrambleVolley.damage)}的伤害。{ToHeal(100 * BrambleVolley.healPercent, "每命中一个目标便会对你治疗", " % 的生命值")}。", "zh-CN");
        ...
        public static void 权杖技能汉化() {
            AddOverlay("ANCIENTSCEPTER_BANDIT2_RESETREVOLVERDESC", Language.GetString("BANDIT2_SPECIAL_DESCRIPTION") + ToScepterDescription($"额外发射1发{ToStack("（每层+1发）")}子弹。"), "zh-CN");
            AddOverlay("ANCIENTSCEPTER_BANDIT2_RESETREVOLVERNAME", "暗杀", "zh-CN");
            AddOverlay("ANCIENTSCEPTER_BANDIT2_SKULLREVOLVERDESC", Language.GetString("BANDIT2_SPECIAL_ALT_DESCRIPTION") + ToScepterDescription($"子弹有25%{ToStack("（每个标记+0.25%）")}概率{ToStack("（不受运气影响）")}弹射到36米{"（每层+12米）".ToStack()}内的最多9名敌人{"（每层+3名）".ToStack()}。\n每次弹射后距离和伤害减少10%。"), "zh-CN");
            AddOverlay("ANCIENTSCEPTER_BANDIT2_SKULLREVOLVERNAME", "叛徒", "zh-CN");
            AddOverlay("ANCIENTSCEPTER_CAPTAIN_AIRSTRIKEALTDESC", Language.GetString("CAPTAIN_UTILITY_ALT1_DESCRIPTION") + ToScepterDescription($"莉莉丝将以{ToUtil(" 自身等级 x 权杖层数 m/s")}的速度追踪附近的敌人，{ToUtil("消耗6层充能")}，{ToUtil("1.5")}倍等待时间，双倍范围，双倍伤害，产生冲击波。"), "zh-CN");
            AddOverlay("ANCIENTSCEPTER_CAPTAIN_AIRSTRIKEALTNAME", "<color=red>PHN-8300“莉莉丝”打击</color>", "zh-CN");
            AddOverlay("ANCIENTSCEPTER_CAPTAIN_AIRSTRIKEDESC", Language.GetString("CAPTAIN_UTILITY_DESCRIPTION") + ToScepterDescription("按住可连续呼叫UES顺风号，总共可造成21x500%伤害。"), "zh-CN");
            AddOverlay("ANCIENTSCEPTER_CAPTAIN_AIRSTRIKENAME", "连续轨道炮", "zh-CN");
            AddOverlay("ANCIENTSCEPTER_COMMANDO_BARRAGEDESC", Language.GetString("COMMANDO_SPECIAL_DESCRIPTION") + ToScepterDescription("向射程内的每个敌人以双倍的速度和精度发射双倍的子弹。按住你的主要技能可以更准确地射击。"), "zh-CN");
            AddOverlay("ANCIENTSCEPTER_COMMANDO_BARRAGENAME", "死亡绽放", "zh-CN");
            AddOverlay("ANCIENTSCEPTER_COMMANDO_GRENADEDESC", Language.GetString("COMMANDO_SPECIAL_ALT1_DESCRIPTION") + ToScepterDescription("扔出10枚具有一半伤害和击退的炸弹。"), "zh-CN");
            AddOverlay("ANCIENTSCEPTER_COMMANDO_GRENADENAME", "地毯式轰炸", "zh-CN");
            AddOverlay("ANCIENTSCEPTER_CROCO_DISEASEDESC", Language.GetString("CROCO_SPECIAL_DESCRIPTION") + ToScepterDescription("使受害者成为行走的瘟疫之源，持续将瘟疫传染给周围的敌人。"), "zh-CN");
            AddOverlay("ANCIENTSCEPTER_CROCO_DISEASENAME", "瘟疫", "zh-CN");
            AddOverlay("ANCIENTSCEPTER_ENGI_TURRETDESC", Language.GetString("ENGI_SPECIAL_DESCRIPTION") + ToScepterDescription("可额外放置1座炮台。"), "zh-CN");
            AddOverlay("ANCIENTSCEPTER_ENGI_TURRETNAME", "TR12-C 高斯自动炮台", "zh-CN");
            AddOverlay("ANCIENTSCEPTER_ENGI_WALKERDESC", Language.GetString("ENGI_SPECIAL_ALT1_DESCRIPTION") + ToScepterDescription("可额外放置2座炮塔。"), "zh-CN");
            ...
            AddOverlay("ANCIENTSCEPTER_HUNTRESS_BALLISTADESC", Language.GetString("HUNTRESS_SPECIAL_ALT1_DESCRIPTION") + ToScepterDescription($"每次射击额外发射3根{"（每层+3根）".ToStack()}弩箭。"), "zh-CN");
            AddOverlay("ANCIENTSCEPTER_HUNTRESS_BALLISTANAME", "-腊包尔->", "zh-CN");
            AddOverlay("ANCIENTSCEPTER_HUNTRESS_RAINDESC", Language.GetString("HUNTRESS_SPECIAL_DESCRIPTION") + ToScepterDescription($"点燃。自动传送到敌人脚下。{ToUtil("1.5")}倍半径和持续时间。{2f.ToUtil()}倍Proc系数。"), "zh-CN");
            AddOverlay("ANCIENTSCEPTER_HUNTRESS_RAINNAME", "火雨", "zh-CN");
            AddOverlay("ANCIENTSCEPTER_LOADER_CHARGEFISTDESC", Language.GetString("LOADER_UTILITY_DESCRIPTION") + ToScepterDescription($"双倍伤害，冲刺速度增加100%{"（每层权杖+100%）".ToStack()}。高到离谱的击退。"), "zh-CN");
            AddOverlay("ANCIENTSCEPTER_LOADER_CHARGEFISTNAME", "百万吨重拳", "zh-CN");
            AddOverlay("ANCIENTSCEPTER_MAGE_FLAMETHROWERDESC", Language.GetString("MAGE_SPECIAL_FIRE_DESCRIPTION") + ToScepterDescription("燃烧将留下灼热的火云。"), "zh-CN");
            AddOverlay("ANCIENTSCEPTER_MAGE_FLAMETHROWERNAME", "龙息", "zh-CN");
            AddOverlay("ANCIENTSCEPTER_MAGE_FLYUPDESC", Language.GetString("MAGE_SPECIAL_LIGHTNING_DESCRIPTION") + ToScepterDescription("双倍伤害，五倍半径，十倍击飞。"), "zh-CN");
            AddOverlay("ANCIENTSCEPTER_MAGE_FLYUPNAME", "反物质浪涌", "zh-CN");
            AddOverlay("ANCIENTSCEPTER_MERC_EVISDESC", Language.GetString("MERC_SPECIAL_DESCRIPTION") + ToScepterDescription($"持续时间增加100%{"（每层权杖+100%）".ToStack()}。此技能击杀敌人可重置持续时间，按住跳跃键可在击杀时退出技能！"), "zh-CN");
            AddOverlay("ANCIENTSCEPTER_MERC_EVISNAME", "屠戮", "zh-CN");
            AddOverlay("ANCIENTSCEPTER_MERC_EVISPROJDESC", Language.GetString("MERC_SPECIAL_ALT1_DESCRIPTION") + ToScepterDescription("刀刃之风的飞行速度和攻击频率增加200%。"), "zh-CN");
            AddOverlay("ANCIENTSCEPTER_MERC_EVISPROJNAME", "死亡之风", "zh-CN");
            AddOverlay("ANCIENTSCEPTER_RAILGUNNER_SNIPECRYODESC", Language.GetString("RAILGUNNER_SPECIAL_ALT_DESCRIPTION") + ToScepterDescription($"击中产生<color=#CCFFFF>冰冻</color>爆炸，<color=#CCFFFF>冰冻</color>{ToUtil("20")}米内的敌人{ToUtil("2")}秒，合计造成{ToDmg(2)}伤害并使其减少{ToUtil("80%")}的移速{ToUtil("20")}秒"), "zh-CN");
            ...
            AddOverlay("ANCIENTSCEPTER_RAILGUNNER_SNIPESUPERDESC", Language.GetString("RAILGUNNER_SPECIAL_DESCRIPTION") + ToScepterDescription($"<color=yellow>发射时，将消耗当前金钱的10%{"（每层权杖+10%）".ToStack()}转化为伤害。</color>击中将永久降低敌人20点护甲。Proc系数增加0.5。"), "zh-CN");
            ...
            AddOverlay("ANCIENTSCEPTER_SCEPLOADER_CHARGEZAPFISTDESC", Language.GetString("LOADER_UTILITY_ALT1_DESCRIPTION") + ToScepterDescription("全向闪电，击中时召唤雷电，合计造成400%<style=cStack>（每层电能钻机+400%）</style>的伤害。冲刺速度增加100%<style=cStack>（每层权杖+100%）</style>，临时提高护甲。<color=#99CCFF>“<link=\"BulwarksHauntShaky\">以雷霆~ 击碎黑暗！</link>”</color>"), "zh-CN");
            AddOverlay("ANCIENTSCEPTER_SCEPLOADER_CHARGEZAPFISTNAME", "<color=#99CCFF>雷霆拳套</color>", "zh-CN");
            AddOverlay("ANCIENTSCEPTER_TOOLBOT_DASHDESC", Language.GetString("TOOLBOT_UTILITY_DESCRIPTION") + ToScepterDescription("进入毁灭模式，可通过跳跃按键退出。将传入的伤害减半（与护甲叠加）。停止后将以巨大的爆炸击晕敌人，合计造成所受伤害200%的伤害。"), "zh-CN");
            AddOverlay("ANCIENTSCEPTER_TOOLBOT_DASHNAME", "毁灭模式", "zh-CN");
            AddOverlay("ANCIENTSCEPTER_TREEBOT_FLOWER2DESC", Language.GetString("TREEBOT_SPECIAL_DESCRIPTION") + ToScepterDescription("双倍范围。造成随机减益。从第二层权杖开始，每层权杖使花朵的持续时间增加100%。"), "zh-CN");
            AddOverlay("ANCIENTSCEPTER_TREEBOT_FLOWER2NAME", "混沌生长", "zh-CN");
            AddOverlay("ANCIENTSCEPTER_TREEBOT_FRUIT2DESC", Language.GetString("TREEBOT_SPECIAL_ALT1_DESCRIPTION") + ToScepterDescription("生成额外的果实，果实给予强大的随机增益。"), "zh-CN");
            AddOverlay("ANCIENTSCEPTER_TREEBOT_FRUIT2NAME", "终极命令：收割", "zh-CN");
            AddOverlay("ANCIENTSCEPTER_VOIDSURVIVOR_CRUSHCORRUPTIONDESC", Language.GetString("VOIDSURVIVOR_SPECIAL_DESCRIPTION") + ToScepterDescription($"技能效果影响周围25米{"（每层+25米）".ToStack()}内的敌人和盟友。"), "zh-CN");
        ...
        public static void 象征汉化() => TPDespair.ZetAspects.Language.tokens["zh-CN"]["ITEM_SHIELDONLY_DESC"] += $"\n<style=cIsLunar>完美的象征：护盾百分比再生速度增加1%{"（每层+1%）".ToStack()}。</style>";// "<color=#CCFFFF>冰霜的象征</color>：" "<color=#99CCFF>雷电的象征</color>："// "<color=#f25d25>火焰的象征</color>：" "<color=#20b2aa>无形的象征</color>："// "<color=#014421>腐蚀的象征</color>：" "<style=cIsLunar>完美的象征</style>："// "<style=cIsHealing>大地的象征</style>："// "<style=cIsVoid>虚空的象征</style>：" "<style=cDeath>忍耐力的象征</style> :"// "<style=cDeath>重力的象征</style> :" "<style=cDeath>模糊的象征</style> :"// "<style=cDeath>愤怒的象征</style> :" "<color=yellow>财富的象征</color>："
    }
    ...
    public class FireFountain : ProjectileFountain {
        ...
        protected override void Fire(FireProjectileInfo info) {
            info.position = transform.position + (victimBody.characterMotor ? (Vector3.up * (victimBody.characterMotor.capsuleHeight * 0.5f + 2f)) : (Vector3.up * (victimBody.bestFitRadius + 2f)));
    ...
    public class M551电塔随特斯拉增强 : MonoBehaviour {
    ...
    public abstract class MissilePool : MonoBehaviour {
        protected CharacterBody _attackerBody;
        protected float _missileTimer;
        protected Dictionary<ProcChainMask, MissileInfo> _pool = new();
        protected virtual GameObject MissilePrefab => GlobalEventManager.CommonAssets.missilePrefab;

        public virtual void AddMissile(float baseDamage, bool isCrit, GameObject target, ProcChainMask procChainMask) {
        ...
        protected void FireMissile(in ProcChainMask procChainMask, in MissileInfo missileInfo) => MissileUtils.FireMissile(_attackerBody.corePosition, _attackerBody, procChainMask, missileInfo.target, missileInfo.baseDamage, missileInfo.isCrit, MissilePrefab, DamageColorIndex.Item, missileInfo.isCrit);
        ...
        protected virtual void OnDestroy() => _pool.Clear();
    ...
    public class ProjectileImpactActionCaller : MonoBehaviour, IProjectileImpactBehavior {
        ...
        private static readonly List<Action<ProjectileImpactActionCaller, ProjectileImpactInfo>> actions = new();
        ...
        public void BindAction(Action<ProjectileImpactActionCaller, ProjectileImpactInfo> actionWaitForAdd) {
            if (actionWaitForAdd != null) {
                actionIndex = actions.Count;
                actions.Add(actionWaitForAdd);
                Helpers.LogInfo("Bind action to projectile :: " + gameObject + "->" + actionWaitForAdd.GetMethodName());
            } else {
                Main.logger_.LogError("Bind action to projectile :: action is null!");
            }
        }
        ...
        public void OnProjectileImpact(ProjectileImpactInfo impactInfo) {
            ...
            actions[actionIndex].Invoke(this, impactInfo);
        }
    }
