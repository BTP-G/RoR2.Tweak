...
namespace BtpTweak {
    ...
    public class Helpers {
        ...
        public static void 基础汉化() {
            ...
            AddOverlay("ENGI_SPECIAL_DESCRIPTION", "放置一个<style=cIsUtility>继承你所有物品</style>的炮塔，发射的炮弹可造成<style=cIsDamage>100%的伤害</style>，最多放置2座。（炮塔会继承浸剂加成，炮塔击杀敌人，所有队员都可以收到浸剂奖励）", "zh-CN");
            AddOverlay("ENGI_SPECIAL_ALT1_DESCRIPTION", "放置一个<style=cIsUtility>移动</style>炮塔可<style=cIsUtility>继承你所有物品</style>。发射的激光可造成<style=cIsDamage>每秒200%的伤害</style>，并可<style=cIsUtility>减速敌人</style>，最多放置2座。（炮塔会继承浸剂加成，塔击杀敌人，所有队员都可以收到浸剂奖励）", "zh-CN");
            ...
            AddOverlay("HUNTRESS_PRIMARY_ALT_DESCRIPTION", $"<style=cIsUtility>灵巧</style>。瞄准60米<style=cStack>（每升一级+{ModConfig.女猎人射程每级增加距离_.Value}米）</style>内的敌人，拉弓射出<style=cIsDamage>{Flurry.minArrows}枚</style>跟踪箭，每枚造成<style=cIsDamage>{d(Flurry.damage)}%的伤害</style>。如果暴击则发射<style=cIsDamage>{Flurry.maxArrows}</style>枚跟踪箭。", "zh-CN");
            AddOverlay("HUNTRESS_PRIMARY_DESCRIPTION", $"<style=cIsUtility>灵巧</style>。瞄准60米<style=cStack>（每升一级+{ModConfig.女猎人射程每级增加距离_.Value}米）</style>内的敌人，快速射出一枚能够造成<style=cIsDamage>{d(Strafe.damage)}%伤害</style>的跟踪箭。", "zh-CN");
            ...
            AddOverlay("ITEM_INFUSION_DESC", $"每击败一名敌人，即可<style=cIsHealing>永久性</style>增加<style=cIsHealing>1</style>点<style=cStack>（每层+1点）</style>生命值，最多增加<style=cIsHealing>自身等级x自身基础血量x0.3x层数</style>点生命值。\n<style=cIsUtility>此物品不会被米斯历克斯拿走</style>。", "zh-CN");
            ...
            AddOverlay("LOADER_SPECIAL_DESCRIPTION", $"扔出飘浮电塔，可<style=cIsDamage>电击</style>周围<style=cIsDamage>{M551Pylon.aoe}</style>米<style=cStack>（每层不稳定的特斯拉线圈+{M551Pylon.aoe}米）</style>内最多<style=cIsDamage>3</style>名敌人，电流最多可弹射{M551Pylon.bounces}次<style=cStack>（每层不稳定的特斯拉线圈+1次）</style>，造成<style=cIsDamage>{d(M551Pylon.damage)}%的伤害</style>。可被<style=cIsUtility>格挡</style>。", "zh-CN");
            ...
            AddOverlay("MAGE_UTILITY_ICE_DESCRIPTION", $"<style=cIsUtility>冰冻</style>。创造一道能够对敌人造成<style=cIsDamage>{d(Snapfreeze.damage)}%伤害</style>的屏障。", "zh-CN");
            ...
            AddOverlay("TOOLBOT_PRIMARY_ALT1_DESCRIPTION", "发射1条具有穿透效果的钢筋，造成<style=cIsDamage>600%</style><style=cStack>（每层三尖匕首+60%）</style>的伤害。", "zh-CN");
            ...
            AddOverlay("TREEBOT_SPECIAL_ALT1_DESCRIPTION", $"发射弩弹，造成<style=cIsDamage>{d(DIRECTIVEHarvest.damage)}%的伤害</style>且弩弹将<style=cIsDamage>注入</style>一个敌人。此敌人死亡时，掉落多个<style=cIsHealing>果实</style>，可治疗<style=cIsHealing>{100 * DIRECTIVEHarvest.percentHeal}%的生命值</style>{(DIRECTIVEHarvest.giveBuffs ? "，并且给予随机<style=cIsHealing>增益</style>" : "")}。", "zh-CN");
            AddOverlay("TREEBOT_SPECIAL_ALT1_NAME", "命令：收获", "zh-CN");
            AddOverlay("TREEBOT_SPECIAL_DESCRIPTION", "<style=cIsHealth>25%生命值</style>。发射一朵会<style=cIsDamage>扎根</style>并造成<style=cIsDamage>200%伤害</style>的花朵。<style=cIsHealing>每命中一个目标便会对你进行一次治疗</style><style=cStack>（每层轻粒子雏菊增加治疗量）</style>。", "zh-CN");
            ...
            AddOverlay("TREEBOT_UTILITY_ALT1_DESCRIPTION", $"<style=cIsHealth>{100 * BrambleVolley.healthCost}%生命值</style>。发射一次<style=cIsUtility>音爆</style>并对敌人造成<style=cIsDamage>{d(BrambleVolley.damage)}%</style>的<style=cIsDamage>伤害</style>。<style=cIsHealing>每命中一个目标便会对你治疗{100 * BrambleVolley.healPercent}%的生命值</style>。", "zh-CN");
        ...
        public static void 权杖技能汉化() {
            ...
            AddOverlay("ANCIENTSCEPTER_CAPTAIN_AIRSTRIKEALTDESC", Language.GetString("CAPTAIN_UTILITY_ALT1_DESCRIPTION") + ScepterDescription("以 自身等级 x 权杖层数 m/s的速度追踪附近的敌人，1.5倍等待时间，2倍范围，100,000%伤害。\n造成枯萎减益。"), "zh-CN");
            AddOverlay("ANCIENTSCEPTER_CAPTAIN_AIRSTRIKEALTNAME", "PHN-8300“<color=red>莉莉丝</color>”打击", "zh-CN");
            ...
            AddOverlay("ANCIENTSCEPTER_RAILGUNNER_SNIPECRYODESC", Language.GetString("RAILGUNNER_SPECIAL_ALT_DESCRIPTION") + ScepterDescription("击中产生<color=#CCFFFF>冰冻</color>爆炸，冰冻6米内的敌人2秒，造成200%伤害并使其减少80%的移速20秒"), "zh-CN");
            ...
            AddOverlay("ANCIENTSCEPTER_SCEPLOADER_CHARGEZAPFISTDESC", Language.GetString("LOADER_UTILITY_ALT1_DESCRIPTION") + ScepterDescription("全向闪电。冲刺速度增加200%<style=cStack>（每层权杖+100%）</style>，临时提高护甲。<color=#99CCFF>“<link=\"BulwarksHauntShaky\">以雷霆~ 击碎黑暗！</link>”</color>"), "zh-CN");
            AddOverlay("ANCIENTSCEPTER_SCEPLOADER_CHARGEZAPFISTNAME", "<color=#99CCFF>雷霆一击·超</color>", "zh-CN");
