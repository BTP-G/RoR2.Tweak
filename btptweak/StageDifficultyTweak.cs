...
namespace BtpTweak {
    ...
    internal class StageDifficultyTweak {
        ...
        private static void Run_AdvanceStage(On.RoR2.Run.orig_AdvanceStage orig, Run self, SceneDef nextScene) {
            if (BtpTweak.是否选择造物难度_ && !nextScene.cachedName.StartsWith("bazaar")) {
                BtpTweak.玩家角色等级生命值系数_ += 1;
                DifficultyCatalog.GetDifficultyDef(self.selectedDifficulty).scalingValue += 1;
                if (ConfigurableDifficulty.ConfigurableDifficultyPlugin.enemyStartingItemList.ContainsKey(RoR2Content.Items.ShinyPearl.itemIndex)) {
                    ConfigurableDifficulty.ConfigurableDifficultyPlugin.enemyStartingItemList[RoR2Content.Items.ShinyPearl.itemIndex] = Mathf.CeilToInt(self.stageClearCount * self.stageClearCount / 4);
                }
            }
            orig(self, nextScene);
        }
