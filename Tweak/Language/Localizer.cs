...
namespace BTP.RoR2Plugin.Language {
    ...
    internal static class Localizer {
        ...
        private static void 基础汉化() {
            AddOverlay("ITEM_JUMPBOOST_DESC", $"在地面上<style=cIsUtility>奔跑时</style><style=cIsUtility>跳跃</style>可以增加<style=cIsUtility>10米</style><style=cStack>（每层增加10米）</style>的距离。<color=#FFFF00>掠夺者特殊效果：跳跃能力提高{RavagerTweak.JumpPowerMultCoefficient.ToBaseAndStkPct()}</color>", "zh-CN");
            ...
            AddOverlay("EQUIPMENT_GOLDGAT_DESC", $"发射一阵钱雨，<style=cIsDamage>每颗子弹均造成{GoldGatFire.damageCoefficient.ToDmgPct()}，外加等同于消耗金钱的{GoldGatTweak.半数.ToBaseAndStkByCloseToPct()}{"（此处层数=成卷的零钱层数+1）".ToStk()}的伤害</style>。每颗子弹消耗{GoldGatFire.baseMoneyCostPerBullet.ToDmg("_枚")}金钱，价格随{"开火时间".ToDmg()}和{"游戏时间".ToUtil()}不断上升。", "zh-CN");
            ...
            AddOverlay("ITEM_ARMORREDUCTIONONHIT_DESC", $"{"2秒".ToUtil()}内命中敌人{"5次".ToDmg()}造成{"粉碎".ToDmg()}减益，将对方<style=cIsDamage>护甲</style>降低<style=cIsDamage>60</style>点，持续<style=cIsDamage>8秒</style><style=cStack>（每层+8秒）</style>。你的攻击将{"穿透".ToDmg()}被{"粉碎".ToDmg()}的敌人{ArmorReductionOnHitTweak.半数.ToBaseAndStkByCloseToPct().ToDmg()}的护甲。", "zh-CN");
            ...
            AddOverlay("ITEM_FIREBALLSONHIT_DESC", $"命中敌人时有{FireballsOnHitTweak.半数.ToBaseAndStkByCloseToPct().ToDmg()}的几率向敌人的{"岩浆球喷泉".ToFire()}中添加{"3颗岩浆球".ToFire() + FireballsOnHitTweak.Interval.ToStk("（喷射间隔：_秒）")}，每颗合计造成{FireballsOnHitTweak.DamageCoefficient.ToBaseAndStkPct().ToDmg("_的总伤害")}，并<style=cIsDamage>点燃</style>敌人。", "zh-CN");
            ...
            AddOverlay("ITEM_LIGHTNINGSTRIKEONHIT_DESC", $"命中敌人时有{LightningStrikeOnHitTweak.半数.ToBaseAndStkByCloseToPct().ToDmg()}的几率向敌人的{"雷电召唤池".ToLightning()}中添加{"1次雷击".ToLightning() + LightningStrikeOnHitTweak.Interval.ToStk("（召唤间隔：_秒）")}，合计造成{LightningStrikeOnHitTweak.DamageCoefficient.ToBaseAndStkPct().ToDmg("_的总伤害")}。<color=#FFFF00>装卸工特殊效果：关于“雷霆拳套”（详情看 权杖 技能介绍）。</color>", "zh-CN");
            ...
            AddOverlay("ITEM_MISSILE_DESC", $"有{MissileTweak.半数.ToBaseAndStkByCloseToPct().ToDmg()}几率向{"导弹发射池".ToUtil()}里添加一枚导弹{MissileTweak.Interval.ToStk("（发射间隔：_秒）")}。每枚合计造成{MissileTweak.DamageCoefficient.ToBaseAndStkPct().ToDmg("_的总伤害")}。", "zh-CN");
        ...
        private static void 权杖技能汉化() {
            ...
            AddOverlay("ANCIENTSCEPTER_HERETIC_SQUAWK_DESC", $"<style=cIsHealth>歌声</style>将标记所有<style=cIsHealth>活体</style>，使其染上<link=\"BulwarksHauntWavy\">{"灭绝".ToRed()}</link>！<link=\"BulwarksHauntWavy\">{"灭绝".ToRed()}</link>：持续{"10秒".ToUtil() + "（每层权杖+10秒）".ToStk()}，当带有<link=\"BulwarksHauntWavy\">{"灭绝".ToRed()}</link>的敌人{"死去".ToRed()}时，会连带着它的{"所有族人".ToRed()}一起{"死去".ToRed()}。", "zh-CN");
            AddOverlay("ANCIENTSCEPTER_HERETIC_SQUAWK_NAME", "<link=\"BulwarksHauntWavy\">灭绝之歌</link>", "zh-CN");
        }
